...
1. **Performance** — card rows per book (champion primary, vt10 secondary): cumulative return, Sharpe since start, MaxDD, current vt scale (or "warming up n/21"), cost drag. Multi-pane chart: dual-line equity rebased to 100, drawdown pane, rolling Sharpe pane, dashed backtest-anchor reference line, 7d/30d/90d/all range pills. Backtest context comes from `champion_backtest.json` as a per-year dev metrics table (SR / return / MaxDD per book), clearly labeled "backtest (dev)" — the file stores yearly aggregates, not a daily equity series, so no backtest curve is drawn.
