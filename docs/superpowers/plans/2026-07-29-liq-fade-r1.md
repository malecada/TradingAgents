...
- **Do not mark any test in this plan `@pytest.mark.slow`.** `pyproject.toml` sets `addopts = "-m 'not slow'"`, so a slow-marked test is deselected by default — it would print "deselected" and read as a pass while verifying nothing. Task 4's coverage tests in particular are the gate that stops a partial fetch from producing a fake verdict, and they must run on every invocation.
...
Expected: `test_every_pct_change_pins_fill_method_none[liq_fade_dev.py]` and `[liq_fade_forensics.py]` FAIL with "pct_change() without explicit fill_method". The `liq_fade_repl.py` case SKIPs (not created until Task 5). `test_anti_vacuity_file_has_pct_change_calls` PASSES for both files. `test_padding_actually_changes_gap_bar_returns` PASSES — it demonstrates the two settings genuinely disagree, it is not the target.
...
Expected: all P3 tests PASS. The pandas-pin test's `liq_fade_repl.py` case now runs instead of skipping and PASSES trivially — the module has no `pct_change` call yet, and "all calls are pinned" is vacuously true over an empty set. The anti-vacuity guard deliberately excludes this file until Task 6 gives it real calls. Nothing is red at the end of this task.
