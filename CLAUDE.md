...
- **Real signed funding**: both backtest engines deduct real per-date Binance funding when asked — `run_coin_backtest(..., funding_series=...)` (V2/V5 path) and `engine.run_backtest(..., funding_series=...)` (V3 / LLM-system path), both SIGNED (long pays, short receives). Wire it via `baseline_v5_mix.run_coin(..., use_real_funding=True)` or `baseline_strategy_v2.py --real-funding`; the funding series comes from `carry_sleeve.funding_daily_income` (daily = SUM of the 3×8h prints) and is clipped to `FUNDING_DAILY_CLIP` to drop venue-cap-exceeding data artifacts (e.g. SOL −17 %/day at the FTX dislocation). Reproduce all corrected headlines: `python scripts/funding_correction_sweep.py`. See THESIS_FINDINGS.md §32.
...
- **Flat funding constant is understated ~14×**: `baseline_v5_mix.COSTS['funding_rate'] = 0.0001/8 ≈ 0.46 %/yr` (a `/8` unit bug; real BTC/ETH funding ≈ 6.6 %/yr). Kept as the default so published numbers stay reproducible (the regression anchor `test_v5_baseline_reproduces_published_sharpe` depends on it); use `use_real_funding=True` for correct absolute returns. Impact is small and **tilt-dependent**: long-biased books lose ~0.03–0.05 SR, two-sided books (V2 symmetric, whose shorts *receive* funding) gain ~0.05 SR, BTC/ETH core ≈ neutral. Do NOT "fix" the constant in place — it would break the reproducibility anchors. See THESIS_FINDINGS.md §32.
