...
- **Market analyst v2 (asset-agnostic "do no harm")**: `config["market_mode"] = "v2"` swaps the 150-indicator free-text legacy market analyst for a structured-snapshot path: deterministic 13-indicator whitelist + ADX/ATR-pct regime tag + category-vote conflict_score + asymmetric LONG (2-of-4 positives, 0 negatives) / SHORT (3-of-4 negatives, 0 positives) default direction, Pydantic-typed LLM output, third-person "Andrew" persona, and per-coin isotonic calibration via `tradingagents/strategies/market_calibration.py`. Modulator consumes `state["market_features"]` mirroring `state["sentiment_features"]`. Code under `tradingagents/market/`; spec at `docs/superpowers/specs/2026-05-26-market-analyst-v2-design.md`; A/B harness at `scripts/run_market_v2_ab.py`.
