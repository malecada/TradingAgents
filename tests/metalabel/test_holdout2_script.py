...
import scripts.metalabel2_holdout as h
...
def test_g3_train_mask_purges_dev_tail_events_touching_holdout():
    # Regression for I1 (mirrored from v1): an event_date <= DEV_END alone
    # is not sufficient — a dev-tail event whose label resolves (touch_date)
    # inside the embargo window right before HOLDOUT_START had its label
    # computed using holdout-window prices. g3_train_mask must exclude it.
