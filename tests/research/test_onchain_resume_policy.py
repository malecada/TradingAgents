...
PATH = Path(__file__).resolve().parents[2]/'research/onchain-graph-2026-09-16/comparison/resume_admission.py'
SPEC = importlib.util.spec_from_file_location('resume_policy_test', PATH)
policy = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(policy)


def fixture():
    dates = [(date(2000, 1, 1)+timedelta(days=n)).isoformat() for n in range(521)]
    missing, completed = dates[:507], dates[507:]
    cohort = dict(dates=missing, prior_complete_dates=sorted(completed + ['old-'+str(n) for n in range(563)]), parent_complete_dates=completed, prefixes={},
                  raw_baseline_bytes=56*1024**3, prior_metadata_baseline_bytes=768*1024**2,
    ...
    baseline = dict(experiments={policy.PARENT:dict(family='old')},
                    families={'old':dict(prior_attempts=8, attempt_budget=9)}, _parent_cohort=dict(prior_complete_dates=['old-'+str(n) for n in range(563)]))
    exp = dict(family='resume', inputs={}, cells=['graph-'+d for d in missing]+['index'])
    exp['outputs'] = [c+'.json' for c in exp['cells']]+['summary.json']
    gate = dict(experiments={policy.EXPERIMENT:exp}, families={'resume':dict(
        prior_attempts=9, attempt_budget=10, mechanism_id='ethereum-bounded-transport-source-resume')})
    certificate = dict(schema_version=1, amendment_id='eth-raw-resume-single-20260917',
        ...
        original_family_cap=9, cumulative_prior_claims=9, additional_claims=1, cumulative_cap=10,
    ...
    review = dict(decision='approve-single-source-resume', target_experiment=policy.EXPERIMENT, additional_claims=1)
...
def test_changed_resume_scope_fails_closed(change):
    f = copy.deepcopy(fixture())
    if change == 'old_budget': f['baseline']['families']['old']['attempt_budget'] = 10
    elif change == 'reset': f['gate']['families']['resume']['prior_attempts'] = 0
