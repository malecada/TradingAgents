...
SOURCE = Path(__file__).resolve().parents[2] / 'research/onchain-graph-2026-09-16/fullpanel_resume2/memory_guard.py'
...
def mock_unit(tmp_path, monkeypatch, *, controls=None, memory=None, completed=False):
    ...
    monkeypatch.setattr(guard, '_read_controls', lambda _: controls or {
        'memory.max': str(6*guard.GIB), 'memory.high': str(6*guard.GIB), 'memory.swap.max': str(guard.GIB//2)})
...
def test_verified_success_and_kernel_command(tmp_path, monkeypatch):
    ...
    launch = calls[0]
    for prop in ['MemoryMax=6442450944', 'MemoryHigh=6442450944', 'MemorySwapMax=536870912', 'KillMode=control-group', 'OOMPolicy=kill']:
...
@pytest.mark.parametrize('change', ['none', 'stale', 'wrong_cgroup', 'wrong_source', 'missing_release', 'wrong_cpu', 'old_high', 'old_namespace'])
def test_worker_requires_live_containment(tmp_path, monkeypatch, change):
    base = tmp_path/'research/onchain-graph-2026-09-16/fullpanel_resume2'
    ...
    if change == 'old_namespace': live['command'][2] = str(base/'run.py').replace('fullpanel_resume2', 'fullpanel_resume')
    ...
    monkeypatch.setattr(guard, '_read_controls', lambda _: {'memory.max': str(6*guard.GIB), 'memory.high': str((4 if change == 'old_high' else 6)*guard.GIB), 'memory.swap.max': str(guard.GIB//2)})
    if change == 'none':
...
def test_equal_high_and_max_are_verified(tmp_path, monkeypatch):
    receipt, calls = mock_unit(tmp_path, monkeypatch, completed=True)
    result = guard.guarded_run(['true'], cwd=tmp_path, receipt_dir=receipt,
                               memory_max_bytes=6*guard.GIB, memory_high_bytes=6*guard.GIB)
    assert result['phase'] == 'complete'
    assert result['kernel_controls']['memory.high'] == result['kernel_controls']['memory.max']


def test_telemetry_optional_absence_is_unknown(tmp_path):
    (tmp_path/'memory.current').write_text('42')
    (tmp_path/'memory.events').write_text('oom 0\noom_kill 0\n')
    value = guard._snapshot(tmp_path)
    unit = value['optional_memory_telemetry']['unit']
    assert value['memory_events'] == {'oom': 0, 'oom_kill': 0}
    assert unit['pressure'] is None and unit['anon_bytes'] is None and unit['swap_current_bytes'] is None
    assert unit['unavailable']


def test_telemetry_records_unit_and_user_ancestor(tmp_path):
    ancestor = tmp_path/'user@1000.service'
    unit = ancestor/'app.slice/probe.service'
    unit.mkdir(parents=True)
    for index, path in enumerate((unit, ancestor), 1):
        (path/'memory.pressure').write_text('some avg10=1.00 avg60=0.50 avg300=0.10 total=123\n')
        (path/'memory.stat').write_text(f'anon {index*100}\nfile {index*200}\n')
        (path/'memory.swap.current').write_text(str(index*300))
    value = guard._telemetry(unit)
    assert value['unit']['anon_bytes'] == 100 and value['user_ancestor']['anon_bytes'] == 200
    assert value['unit']['file_bytes'] == 200 and value['user_ancestor']['swap_current_bytes'] == 600
    assert value['unit']['unavailable'] == value['user_ancestor']['unavailable'] == {}
