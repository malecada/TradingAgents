...
from tradingagents.research_options_capture import adapter as a
from tradingagents.research_options_capture.schedule import request,DAY,HOUR
...
def receipt(recipe,value,at):
    ...
    return {'status':'received','request':recipe,'body_base64':base64.b64encode(raw).decode(),'body_bytes':len(raw),'body_sha256':sha,
            'metadata':{'attempted':True,'body_complete':True,'clock_consistent':True,'within_controller_deadline':True,'http_status':200,'error':None,
                        ...
                        'controller_deadline_ms':at+5000,'controller_deadline_monotonic_ns':(at+5000)*1000000}}


def initial():
    options,futures=metadata();known={}
    for key,spec in a.known_requests().items():
        if spec['kind']=='time':value={'serverTime':T+250}
        elif spec['kind']=='index':value={'indexPrice':'100','time':T+250}
        elif spec['kind']=='depth':value={'T':T+200,'E':T+250,'lastUpdateId':1,'bids':[['100','100']],'asks':[['100.1','100']]}
        else:value={'symbol':spec['parameters']['symbol'],'markPrice':'100','time':T+250,'nextFundingTime':T+8*HOUR}
        known[key]=receipt(a._recipe(spec),value,T)
