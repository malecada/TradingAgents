...
from tradingagents.research_options_capture import schedule, worker
...
class ScheduleTests(unittest.TestCase):
    def test_exact_denominator_windows_and_routes(self):
        ...
        for rows in slots.values():
            ...
            for s in rows:
                self.assertEqual(s['deadline_ms']-s['scheduled_ms'],5000)
                self.assertTrue(request_url(s['request']).startswith('https://'))
        self.assertEqual(slots['bootstrap'][0]['scheduled_ms'],T-60000)
        self.assertEqual(slots['known'][-1]['scheduled_ms'],T+44*schedule.DAY)
        final=slots['final']
...
class WorkerTests(unittest.TestCase):
    def execute(self, *, deny=False, unavailable=False, start_offset=-60000, real_adapter=False, restart=False):
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch.object(worker,'assignment',return_value=spec):
                if restart:
    ...
    def test_frozen_package_and_claim_clock_binding(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            claim={'source':'b'*40,'episode_protocol':{'worker_lease':{'not_before':iso(T-120000),'expires_at':iso(T+45*schedule.DAY+60000)},
            ...
            value={'schema_version':1,'entry_ms':T,'lease_not_before_ms':T-120000,'lease_expires_ms':T+45*schedule.DAY+60000,
                   ...
                   'target':'options-episode-20260911','source_commit':'b'*40,'data_root':str(p/'data'),'host_identity':os.uname().nodename}
            raw=encode(value);(p/'assignment.json').write_bytes(raw)
            with patch.object(worker,'__file__',str(p/'tradingagents/research_options_capture/worker.py')):
                ...
                extra.unlink()
                dest=p/'tradingagents/research_options_capture/schedule.py';dest.write_bytes(b'# altered\n')
    ...
    def test_actual_bootstrap_forwards_isolated_child_arguments(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            for name in worker.PACKAGE_FILES:
                ...
                body=(root/'scripts/options_release_bootstrap.py').read_bytes() if name=='release_bootstrap.py' else b'# invented module\n'
                if name=='tradingagents/research_options_capture/worker.py':
