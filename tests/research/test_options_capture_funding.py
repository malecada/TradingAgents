...
import pytest
from tests.research.test_options_capture_adapter import receipt, T
from tradingagents.research_options_capture import funding as f
from tradingagents.research_options_capture.schedule import calendar,HOUR,DAY
...
def fixture(events=None):
    ...
    for spec in slots['known']:
        ...
        elif spec['id'].endswith('-btc-perp-mark'):value={'symbol':'BTCUSDT','time':at+250,'nextFundingTime':at+HOUR}
        else:continue
        out['known'][spec['id']]=receipt(spec['request'],value,at)
...
def replace(data,group,name,value):
    ...
    data[group][name]=receipt(spec['request'],value,spec['scheduled_ms'])
...
def test_missing_type_requires_actual_bootstrap_coin_identity():
    ...
    bootstrap=receipt(spec['request'],value,spec['scheduled_ms'])
...
def test_empty_unknown_and_conditionally_known_no_event_are_distinct():
    ...
    data=fixture([])
    for h in (0,1):replace(data,'known',f'h{h:04d}-btc-perp-mark',{'symbol':'BTCUSDT','time':T+h*HOUR+250,'nextFundingTime':T+8*HOUR})
    known=run(data)
...
def test_next_event_after_provider_time_but_not_future_at_arrival_is_unknown():
    data=fixture()
    replace(data,'known','h0001-btc-perp-mark',{'symbol':'BTCUSDT','time':T+HOUR+250,'nextFundingTime':T+HOUR+260})
    result=run(data)
