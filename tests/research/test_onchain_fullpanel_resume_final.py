...
SPEC = importlib.util.spec_from_file_location('resume_final_synthetic', ROOT / 'research/onchain-graph-2026-09-16/fullpanel_resume/check_final.py')
...
def resource_fixture(root):
    return dict(cwd=str(root), command=[c.PINNED_PYTHON,'-B',str(root/c.BASE/'run.py'),'--source','a'*40],
                memory_max_bytes=6*2**30, memory_high_bytes=4*2**30, memory_swap_max_bytes=2**29,
                ...
                'memory.high':str(4*2**30),'memory.swap.max':str(2**29)},
