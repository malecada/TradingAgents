...
import pytest
from tradingagents.research_options_capture import returned as r
from tradingagents.research_options_capture import worker,adapter
from tradingagents.research_options_capture.journal import Journal,encode,digest
from tradingagents.research_options_capture.schedule import calendar,DAY,ASSETS,groups
from tests.research.test_options_capture_adapter import T,initial,metadata,receipt,selected
...
def package(root):
    ...
    claim={'source':'a'*40,'episode_protocol':{'worker_lease':{'not_before':stamp(T-120000),'expires_at':stamp(T+45*DAY+60000)},'observation_window':{'start':stamp(T),'end':stamp(T+44*DAY+5000)}}}
    ...
    value={'schema_version':1,'entry_ms':T,'lease_not_before_ms':T-120000,'lease_expires_ms':T+45*DAY+60000,
           ...
           'authority':'raw-worker-only','target':'options-episode-20260911','source_commit':'a'*40,'data_root':'/invented/vps/research-data','host_identity':'invented-vps-host'}
...
def populate(p,data,anchor,spec,*,successful_selection=False,sealed=True):
    ...
    if sealed:
        ...
        value={'assignment_sha256':anchor,'status':'failed','reason':'invented stop','journal_seals':seals,'intended_slot_count':17144,
               ...
               'selection_sha256':r._hash(data/'selection.json') if chosen else None,'quiescent_ms':T+1000,
...
def test_unsafe_or_changed_return_refused(tmp_path,fault):
    ...
    elif fault=='source':(p/'tradingagents/research_options_capture/adapter.py').write_bytes(b'changed')
