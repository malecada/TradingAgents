...
import pytest
from tests.research.test_options_capture_control import build_fixture,start,write,ref,rebind
from tradingagents.research_options_capture import control,worker
ROOT=Path(__file__).resolve().parents[2]
spec=importlib.util.spec_from_file_location('options_release_builder_test',ROOT/'scripts/options_release_package.py')
...
def test_release_binds_committed_sources_and_refuses_replacement(tmp_path):
    ...
    dest=tmp_path/'release';data='/opt/thesis-research/options-episode-20260911/data'
    ...
    (root/'scripts/options_release_bootstrap.py').write_text('# changed after claim')
