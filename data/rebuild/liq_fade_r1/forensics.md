...
1892 masked triggers across 304 band symbols over 4.24 years of dev window (446 events/year, 6.22 events per band symbol across the full 304-symbol band). 219 of those symbols registered at least one event — 8.64 events per *active* symbol, the like-for-like comparison against `liq_fade_i1`'s 710 events over its 88 active symbols (8.07 events/symbol there): comparable order of magnitude on an apples-to-apples, active-symbol basis. 1884 events have a full H=48 forward window and enter the mean below.
...
## Pooled single-symbol concentration — disclosure only, verdict unchanged

The pooled P2 statistic above sums to **-7.87pp** across all 1884 events. A single symbol, **FTTUSDT**, contributes **-12.61pp** of that total from just 27 events — 160% of the pooled sum, and more negative than the pooled total itself. Excluding it, the pooled mean becomes **+0.2552%** over 1857 events (t=0.70), which is **ABOVE** the pre-registered +0.25% P2 floor.

**This figure is reported, not acted on.** The registered P2 statistic is the full-band mean computed over *all* events (the Power section above); `data/rebuild/gates.json`'s `liq_fade_r1.stop_rule` explicitly forbids "no cost-model relaxation" and a second pass after a probe failure, and post-hoc exclusion of a single symbol chosen because it is the largest contributor is exactly the kind of relaxation that rule exists to prevent — it is indistinguishable in kind from picking the best cell in an unregistered grid. The verdict is **not** recomputed on this basis and remains **NEGATIVE**. The finding is reported here because the negative rests substantially on one delisted exchange token (FTTUSDT, the FTX token, permanently impaired by the November 2022 FTX collapse) rather than on a broad-based pattern, and a reader deserves to know that; the pre-registration is precisely what stops this number from being used to rescue the result.
