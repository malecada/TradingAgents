...
from tradingagents.research_options_capture.schedule import calendar,DAY,HOUR,groups
...
def main(report_path,resume_root=None):
    if resume_root is not None:
        ...
        temp=Path(tempfile.mkdtemp(prefix='options-full-episode-'));package=temp/'package';package.mkdir();data=temp/'data'
        files={}
        for name in worker.PACKAGE_FILES:
            source=ROOT/('scripts/options_release_bootstrap.py' if name=='release_bootstrap.py' else name)
        ...
        claim={'source':'b'*40,'authority':'local-master-only','synthetic':True,'episode_protocol':{'worker_lease':{'not_before':iso(t-120000),'expires_at':iso(t+45*DAY+60000)},'observation_window':{'start':iso(t),'end':iso(t+44*DAY+5000)}}}
        ...
        assignment={'schema_version':1,'entry_ms':t,'lease_not_before_ms':t-120000,'lease_expires_ms':t+45*DAY+60000,
            ...
            'authority':'raw-worker-only','target':'options-episode-20260911','source_commit':'b'*40,'data_root':str(data),'host_identity':os.uname().nodename}
    ...
    def checkpoint(stage):
        progress.write(json.dumps({'stage':stage,'fixture_root':str(temp),'assignment_sha256':expected,'elapsed_seconds':time.monotonic()-started,'synthetic_requests':len(attempts),'groups':groups_seen,'clock_ms':clock[0],'prior_consumed_intents':initial_consumed,'inventory_before_sha256':digest(before_path.read_bytes())})+'\n');progress.flush();os.fsync(progress.fileno())
    checkpoint('start')
    def sleep(unused):
    ...
    def acquire(journal,names,*,deadline_ms):
        ...
        for name in names:
            ...
            slot=journal.slots[name];at=slot['scheduled_ms'];body=json.dumps(payload(slot),separators=(',',':')).encode();sha=digest(body)
            ...
            meta={'request_url':request_url(slot['request']),'child_started':True,'attempted':True,'request_ms':at+200,'request_monotonic_ns':(at+200)*1000000,
                ...
                'controller_window_start_ms':at,'controller_window_start_monotonic_ns':at*1000000,'controller_deadline_ms':at+5000,'controller_deadline_monotonic_ns':(at+5000)*1000000,
    ...
    with patch.object(worker,'__file__',str(package/'tradingagents/research_options_capture/worker.py')):
    ...
    result={'scope':'Full synthetic worker lifecycle only; no HTTP or financial evaluation.','source_hashes':files,'fixture_root':str(temp),'assignment_sha256':expected,
