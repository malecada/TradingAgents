...
from tradingagents.research_options_timing.schedule import calendar,groups,ASSETS,DAY
...
def metadata(slot,raw):
    at=slot['scheduled_ms'];mono=2**63-1;span=(slot['deadline_ms']-at)*1000000
    return {'request_url':request_url(slot['request']),'child_started':True,'attempted':True,
            'request_ms':at,'request_monotonic_ns':mono-span,'http_status':200,
            'body_complete':True,'error':'\uffff'*300,'retrieval_ms':slot['deadline_ms'],
            'retrieval_monotonic_ns':mono,'body_bytes':len(raw),'body_sha256':digest(raw),
            'controller_retrieval_ms':slot['deadline_ms'],'controller_retrieval_monotonic_ns':mono,
            'controller_window_start_ms':at,'controller_window_start_monotonic_ns':mono-span,
            'controller_deadline_ms':slot['deadline_ms'],'controller_deadline_monotonic_ns':mono,
            'child_exit_code':-999999,'clock_consistent':True,'within_controller_deadline':True}


def encoded_members(slot,raw):
    name=slot['id']
    intent=encode({'kind':'intent','claim_sha256':CLAIM,'slot':name,'request':slot['request'],'now_ms':slot['deadline_ms']})
...
def main(report_path):
    report={'scope':'Invented engineering only; no HTTP, empirical claim or economic result.','source_hashes':{n:digest(Path(worker.__file__).with_name(n+'.py').read_bytes()) for n in ('journal','schedule','worker','transport','adapter')},'started_wall_ms':time.time_ns()//1000000}
    ...
    with tempfile.TemporaryDirectory(prefix='options-timing-resource-') as temp:
        ...
        for row in known.values():
            ...
            fresh=fixture.receipt(row['request'],value,fixture.T+2000)
            fresh['metadata']['controller_deadline_ms']=fixture.T+5000
            fresh['metadata']['controller_deadline_monotonic_ns']=(fixture.T+5000)*1000000
            row.clear();row.update(fresh)
        ...
        with contextlib.ExitStack() as stack:
            ...
            # Timed three-second acquisition critical section has no HTTP latency. It includes
            ...
            kn.begin_group(names,now_ms=fixture.T+2000)
            for n in names:
            ...
            sj.recover(now_ms=fixture.T+2000)
            selected_names=groups(generated['selected'])[0][1];sj.begin_group(selected_names,now_ms=fixture.T+2000)
            ...
            report['entry_remaining_of3_seconds_before_network']=3-report['entry_local_critical_seconds']
    ...
    report['conclusions']={'serialized_calendar_fits_proposed_caps':all(r['all_received_plus_one_staging_fits_nonterminal'] for r in report['arithmetic']['journals'].values()),
        'local_initialization_under3seconds_without_network':report['entry_local_critical_seconds']<3,
