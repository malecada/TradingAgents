...
def tree_rss(pid):
    """Sum leader-thread descendants; financial runner forbids detached/background children."""
    try:
        status = Path(f'/proc/{pid}/status').read_text()
        children = Path(f'/proc/{pid}/task/{pid}/children').read_text().split()
    except FileNotFoundError:
        return 0  # A sampled process may exit between tree discovery and read.
    values = [line.split()[1] for line in status.splitlines() if line.startswith('VmRSS:')]
    if not values:  # Only an exited zombie is allowed to lack resident memory.
        if any(line.startswith('State:') and 'Z' in line for line in status.splitlines()):
            return 0
        raise RuntimeError('live process has no VmRSS field')
    return int(values[0]) * 1024 + sum(tree_rss(int(child)) for child in children)
...
def run_guard(command, *, rss_limit_bytes=512*1024**2, wall_seconds=120, interval=.02):
    ...
    return {'child_exit_code': process.wait(), 'limit_reason': reason,
            ...
            'qualification': '20 ms sampled aggregate RSS; brief overshoot can occur. No detached/background or secondary-thread subprocesses admitted. Shared pages may be counted twice. ru_maxrss is the largest individual child peak, not aggregate.'}
