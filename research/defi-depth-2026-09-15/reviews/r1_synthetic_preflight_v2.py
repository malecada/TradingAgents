...
def main():
    fixture = load('invented_q2_fixture', ROOT/'tests/research/test_defi_depth_repair_v2.py')
    ...
    with tempfile.TemporaryDirectory(prefix='defi-r1-invented-') as directory:
        ...
        real = json.loads((HERE/'gates-r1-v2.json').read_text())
        exp = copy.deepcopy(real['experiments']['defi-depth-r1-20260915'])
        for name in exp['source_files']:
            dest = root/name
            dest.parent.mkdir(parents=True, exist_ok=True)
            shutil.copyfile(ROOT/name, dest)
        shutil.copyfile(HERE/'r1-charter-v2.md', here/'r1-charter-v2.md')
        ...
        (here/'gates-r1-v2.json').write_text(json.dumps(gate, indent=2)+'\n')
        ...
        module = load('isolated_r1', here/'r1_source_v2.py')
        ...
        assert len(calls) == 8
        receipt = {'scope': 'Invented source states only; compact real R1 main/research_defi_repair lifecycle (unamended invented family) with injected transport plus complete 1096-day invented capture in memory; no actual network/market inputs',
    ...
    (HERE/'reviews/r1-synthetic-preflight-v2.json').write_text(json.dumps(receipt, indent=2)+'\n')
