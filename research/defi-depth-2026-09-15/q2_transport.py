"""Q2-only bounded public Base batch transport; no experiment duration cutoff."""
import json
import http.client
import signal
import urllib.error
...
def public_post(url, payload):
    ...
    if not 1 <= len(members) <= 32:
        raise ValueError('unregistered batch size')
    ...
    try:
        ...
        request = urllib.request.Request(url, data=encoded, method="POST",
            headers={"Accept": "application/json", "Content-Type": "application/json",
                     "User-Agent": "RegisteredDefiDepthSource/2.0"})
        try:
