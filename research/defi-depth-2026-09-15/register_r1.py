...
s=importlib.util.spec_from_file_location('register_r1_source',HERE/'r1_source.py');r1=importlib.util.module_from_spec(s);s.loader.exec_module(r1)
...
exp['charter']=reference(base+'r1-charter.md');exp['runtime_hashes']=runtime_hashes()
exp['source_files'].pop(base+'q2_source.py');exp['source_files'][base+'r1_source.py']=digest(base+'r1_source.py')
...
'baseline_harness_source_files':{base+'q2_source.py':parent['source_files'][base+'q2_source.py']},
 'target_harness_source_files':{base+'r1_source.py':exp['source_files'][base+'r1_source.py']},
'experiment_invariants':{k:v for k,v in parent.items() if k not in MUTABLE}}
write(base+'r1-preservation-manifest.json',manifest)
contract_sha=sha(canonical(exp));manifest_sha=digest(base+'r1-preservation-manifest.json')
if (HERE/'reviews/r1-synthetic-preflight.json').exists():
    write(base+'reviews/r1-repair-preflight.json',{'status':'pass','target_experiment':r1.EXPERIMENT,
    'economic_manifest_sha256':manifest_sha,'reports':[reference(base+'reviews/r1-synthetic-preflight.json')]})
claims={}
...
'prior_claims':claims,'review':reference(base+'reviews/r1-amendment-approval.json'),
'repair_preflight':reference(base+'reviews/r1-repair-preflight.json'),
'economic_manifest':reference(base+'r1-preservation-manifest.json')}
write(base+'r1-budget-amendment.json',cert)
exp['budget_amendment']=reference(base+'r1-budget-amendment.json')
gate['experiments'][r1.EXPERIMENT]=exp
write(base+'gates-r1.json',gate)
