"""Fixed daily Base protocol history; source observations only, no returns."""
import argparse
import base64
...
ROOT = HERE.parents[1]
EXPERIMENT = 'defi-depth-q2-20260915'
REGISTRATION = 'research/defi-depth-2026-09-15/gates-q2.json'
...
def outputs_for(spec):
    ...
    for day in spec['days']:
        names += [day['date']+'-'+name+'.json' for name in ('headers-attempt','headers-receipt','state-attempt','state-receipt','results')]
...
def capture(spec, publish, fetch=transport.public_post):
    ...
    for n, day in enumerate(spec['days'], 1):
        ...
        found, reason, _ = request(prefix+'state', payload, missing or (None if block else 'canonical bracket unavailable'))
        values = {}
        for action in actions:
            row = decode(prefix+action['key'], found, reason, lambda value: parse_action(value, action))
...
def main():
    ...
    with ResearchRun.start(root=ROOT,registration=REGISTRATION,experiment=EXPERIMENT,source=args.source) as run:
        ...
        if phase['slots']['Q2'] != {'kind':'source','experiment':EXPERIMENT} or phase['elapsed_time_kill'] is not False:
