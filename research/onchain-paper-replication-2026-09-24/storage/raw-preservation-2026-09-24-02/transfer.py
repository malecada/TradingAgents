...
def worker(phase):
    ...
    try:
        ...
        if phase!='pilot':
            ...
            if release['contract_sha256']!=contract_hash or release['pilot_complete_sha256']!=sha256(HERE/'pilot/complete.json') or release['status']!='released':
...
def run_phase(phase,c,remaining):
    ...
    result=guarded_run([str(ROOT/'.venv/bin/python'),'-B',str(Path(__file__).resolve()),'--worker',phase],
        ...
        memory_swap_max_bytes=0,reserve_bytes=3*GIB,start_reserve_bytes=int(3.25*GIB),
...
if __name__=='__main__':
    ...
    if len(sys.argv)!=2 or sys.argv[1] not in ('pilot','all-bulk'):raise ValueError('choose pilot or all-bulk')
    mode=sys.argv[1];c,ch,_=load_contract()
    with (HERE/'controller.lock').open('a') as lock:
        fcntl.flock(lock,fcntl.LOCK_EX|fcntl.LOCK_NB)
        if mode=='pilot':raise SystemExit(0 if run_phase('pilot',c,c['phases']['pilot']['wall_seconds']) else 1)
        release=json.loads((HERE/'BULK_RELEASE.json').read_bytes())
        if (release['status']!='released' or release['contract_sha256']!=ch
                or release['pilot_complete_sha256']!=sha256(HERE/'pilot/complete.json')):
        ...
        try:
            ...
            records=[json.loads((HERE/name/'complete.json').read_bytes()) for name in ['pilot',*phases]]
