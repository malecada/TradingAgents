"""Finite continuation05: preserve prior attempts and retain bounded diagnostics."""
...
from tradingagents.research.onchain_replication.preservation import plan_batches,sha256,transfer_bundle
...
def reused_prefix(c, batches):
    """Only an exact, verified prefix may be excluded from this continuation."""
    first = min(p['start'] for p in c['phases'].values())
    if [r['index'] for r in c['reused_batches']] != list(range(1, first)):
        raise ValueError('reused prefix has a gap, duplicate or overlap')
    records = []
    for reuse in c['reused_batches']:
        name = reuse['complete_path']; raw = (ROOT/name).read_bytes()
        if hashlib.sha256(raw).hexdigest() != c['input_sha256'][name]:
            raise ValueError('reused bundle evidence changed')
        record = json.loads(raw); batch = batches[reuse['index']]
        if (record['status'] != 'complete'
                or any(record[k] is not True for k in
                       ('originals_preserved', 'downloaded_members_verified', 'source_hashes_verified'))
                or record['files'] != batch['stop']-batch['start']
                or record['raw_bytes'] != batch['raw_bytes']
                or record['start_index'] != batch['start']):
            raise ValueError('reused bundle is unverified or differs from batch plan')
        records.append(record)
    return {'files': sum(r['files'] for r in records),
            'raw_bytes': sum(r['raw_bytes'] for r in records),
            'reused_batch_indices': [r['index'] for r in c['reused_batches']]}


def check_local_scratch(c):
    if shutil.disk_usage(ROOT).free < c['local_free_floor_bytes']+c['maximum_temporary_bytes']:
        raise RuntimeError('insufficient local floor plus packing/readback scratch')


def receive_diagnostic(command, destination, *, expected_bytes, max_seconds, bytes_per_second):
    """Bound stdout as before, draining stderr concurrently into a 16 KiB tail."""
    ...
    with Path(destination).open('xb') as output:
        try:
            ...
            while streams:
                ...
                for fd in ready:
                    chunk = os.read(fd, 65536 if fd == stderr else min(65536, expected_bytes-received+1))
                    if not chunk:
                        del streams[fd]
                        continue
                    if fd == stderr:
                        seen += len(chunk); tail = (tail+chunk)[-16384:]
                    else:
                        received += len(chunk)
            ...
            if proc is not None:
                ...
                # Preserve queued error bytes when the data bound/deadline broke the loop.
                try:
            ...
            _immutable(receipt, {'status': 'failed' if error_type else 'complete',
                'error_type': error_type, 'pid': proc.pid if proc else None,
...
class Transport:
    ...
    def run(self,args):
        ...
        try:
            ...
            print(json.dumps({'transport_error_type':type(error).__name__,
                'returncode':getattr(error,'returncode',None),
                'stderr_tail':(error.stderr or b'')[-16384:].decode('utf-8',errors='replace')}),
    ...
    def get(self,path,destination):
        ...
        receive_diagnostic([*self.ssh,'dd','if='+self.remote_path(path),'bs=32768','count='+str(count)],
...
def worker(phase):
    ...
    try:
        ...
        reused_pilot(c)
        reused_prefix(c, batches)
        check_local_scratch(c)
...
def run_phase(phase,c,remaining):
    ...
    result=guarded_run([str(ROOT/'.venv/bin/python'),'-B',str(Path(__file__).resolve()),'--worker',phase],
        ...
        disk_paths=[ROOT],disk_floor_bytes=c['local_free_floor_bytes'],wall_seconds=min(c['phases'][phase]['wall_seconds'],remaining))
...
if __name__=='__main__':
    ...
    with (HERE/'controller.lock').open('a') as lock:
        ...
        reused_pilot(c)
        prefix = reused_prefix(c, json.loads((HERE/'batches.json').read_bytes()))
        check_local_scratch(c)
        release=json.loads((HERE/'BULK_RELEASE.json').read_bytes())
        ...
        try:
            ...
            records=[reused_pilot(c),prefix,*[json.loads((HERE/name/'complete.json').read_bytes()) for name in phases]]
            ...
            _immutable(controller/'complete.json',{'status':'complete','phases':completed,'files_including_pilot':total_files,
                ...
                'scope':c['scope'],'originals_preserved':True,'reused_batch_indices':prefix['reused_batch_indices']})
        except BaseException as error:
