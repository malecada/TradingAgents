...
def main():
    target = Path('/tmp/onchain-paper-generic-job-synthetic-20260924-02')
    target.mkdir(exist_ok=False)
    reg, populations, plan = setup(registered.__wrapped__(target))
    _, spec, _ = reg
    experiment = spec['experiments'].pop('example-a')
    identity = 'controller-synthetic-20260924-02'
    ...
    destination = ROOT/'research/onchain-paper-replication-2026-09-24/full_sources/controller-synthetic-02-preparation.json'
