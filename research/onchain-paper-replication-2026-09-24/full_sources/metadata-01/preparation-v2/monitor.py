...
if __name__=='__main__':
    ...
    ownership=ROOT/'research_artifacts/onchain-paper-replication-2026-09-24/pilot-01-supervisor'
    ...
    command=[sys.executable,'-B',str(HERE/'worker.py'),'--source',args.source]
    result=guarded_run(command,cwd=ROOT,receipt_dir=receipt,disk_paths=[ROOT],wall_seconds=1800,memory_max_bytes=1024**3,memory_high_bytes=768*1024**2,memory_swap_max_bytes=0,reserve_bytes=3*1024**3,owner_identity=identity)
    ...
    observed=module.reconcile(receipt,receipt.parent/'sources'/'paper-full-source-metadata-20260924',args.source,identity)
