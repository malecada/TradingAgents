...
GUARD=HERE/'preparation-recovery-01-guard'
...
def main():
    ...
    contract=json.loads((HERE/'preparation-recovery-01-contract.json').read_bytes())
    ...
    _immutable(output/'intent.json',{'contract_sha256':file_hash(HERE/'preparation-recovery-01-contract.json'),'started_at':datetime.now(timezone.utc).isoformat()})
    def fetch(item):
        name,info=item
    ...
    _immutable(output/'result.json',record);_immutable(HERE/('preparation-recovery-01-'+status+'.json'),record)
