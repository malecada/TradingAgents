...
GUARD=HERE/'preparation-recovery-06-guard'
...
def main():
    ...
    contract=json.loads((HERE/'preparation-recovery-06-contract.json').read_bytes())
    ...
    _immutable(output/'intent.json',{'contract_sha256':file_hash(HERE/'preparation-recovery-06-contract.json'),'started_at':datetime.now(timezone.utc).isoformat()})
    def fetch(item):
        name,info=item
    ...
    record={'status':status,'commit':contract['commit'],'verified_members':sum(x['status']=='complete' for x in results),
        ...
        'output':str(output),'results':results,'qualification':'supplemental three log files omitted by Git ignore rules; checkpoint05 remains immutable; earlier base chain separately recovered; full raw transaction backup unverified'}
    _immutable(output/'result.json',record);_immutable(HERE/('preparation-recovery-06-'+status+'.json'),record)
