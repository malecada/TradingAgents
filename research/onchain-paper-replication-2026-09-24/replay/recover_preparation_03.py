...
GUARD=HERE/'preparation-recovery-03-guard'
...
def main():
    ...
    contract=json.loads((HERE/'preparation-recovery-03-contract.json').read_bytes())
    ...
    _immutable(output/'intent.json',{'contract_sha256':file_hash(HERE/'preparation-recovery-03-contract.json'),'started_at':datetime.now(timezone.utc).isoformat()})
    def fetch(item):
        name,info=item
    ...
    record={'status':status,'commit':contract['commit'],'verified_members':sum(x['status']=='complete' for x in results),
        ...
        'output':str(output),'results':results,'qualification':'exact reviewed snapshot03 delta bundle and manifest; base652 separately recovered; full raw backup remains unverified'}
    _immutable(output/'result.json',record);_immutable(HERE/('preparation-recovery-03-'+status+'.json'),record)
