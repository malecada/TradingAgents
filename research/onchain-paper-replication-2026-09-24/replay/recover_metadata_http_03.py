...
GUARD=HERE/'remote-metadata-recovery-03-guard'
...
def main():
    ...
    contract=json.loads((HERE/'remote-metadata-recovery-03-contract.json').read_bytes())
    ...
    _immutable(output/'intent.json',{'contract_sha256':file_hash(HERE/'remote-metadata-recovery-03-contract.json'),'started_at':datetime.now(timezone.utc).isoformat()})
    def fetch(item):
        name,info=item
        if Path(name).is_absolute() or '..' in Path(name).parts or not name.startswith(('research_runs/paper-full-source-metadata-20260924/','research_artifacts/onchain-paper-replication-2026-09-24/')):raise ValueError('unregistered recovery member')
    ...
    record={'status':status,'commit':contract['commit'],'verified_members':sum(x['status']=='complete' for x in results),
        ...
        'output':str(output),'results':results,'qualification':'raw metadata and compact receipts only; no transaction-body or empirical-model backup claim'}
    _immutable(output/'result.json',record);_immutable(HERE/('remote-metadata-recovery-03-'+status+'.json'),record)
