...
- **No active empirical jobs.** `eth-paper-resource-pilot-20260924-02` is
  ...
  graph build: ancestor-user memory pressure51.89% exceeded50% forover20seconds.
  Guard10247.694seconds, peak sampled2,783,363,072bytes,231224memory.high events,
  zero kernel cgroupOOM counters. Cleanup verified and all ownedPIDs absent.
  All109cell dispositions retained:7complete/102unavailable. The largest-week
  graph did not complete. See pilot_successor_02/closure-01/closure.json and
  scoped journal evidence. Independent closure review is complete: all 69 indexed
  artifact hashes matched, including 17 large files totaling 5,305,932,810 bytes.
  All 63 source-day ledger cells remain unavailable because finalization was
  interrupted; graph receipts nevertheless prove 14 decoded days, and the third
  week was attempted. Those exposed inputs are not fresh or never attempted.
...
- Task1 complete: public paper/artifact audit, explicit assumptions, all44 table
  rows,17-claim history, proposed cumulative51 budget, original protocol freeze.
  Float64 internal matching amendment and v2 freeze preserve original bytes.
  `check_protocol.py` checks identities and arithmetic; it does not admit a run.
- Tasks2–3 software implemented and independently reviewed: contracts, durable
  immutable cache, ETH readers, complete-week graphs, offline Yahoo parser and
  bounded capture interface. Synthetic checks pass; real source completeness,
  capture admission and empirical C03/C04 are still pending.
- Task4 reference and batched tensor literal paper matcher implemented; independent
  high-precision oracle checked. CPU checks pass. CUDA comparison C06 unavailable
  on this host; full-size performance remains unmeasured.
- Task5 dictionary/MCM implemented with deterministic sampling, average linkage,
  medoids, partition hierarchy and immutable identities. Review corrections cover
