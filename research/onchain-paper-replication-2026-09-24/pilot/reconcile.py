...
EXPERIMENT='eth-paper-resource-pilot-20260924'
...
def reconcile(receipt,artifacts,source,owner_identity):
    ...
    if claim['source']!=source:raise ValueError('observer claim source differs')
    ...
    recorded={}
    for p in sorted(artifacts.glob('cell-*.json')):
        row=json.loads(p.read_bytes());recorded[row['id']]=row
    cells=[]
    for name in claim['experiment']['cells']:
        row=recorded.get(name)
        if row is None:
            phase,week=name.rsplit('-',3)[0],'-'.join(name.rsplit('-',3)[1:])
            path=artifacts/week/phase/'result.json'
            if path.exists():row={**json.loads(path.read_bytes()),'id':name}
        cells.append(row or {'id':name,'status':'unavailable','reason':'workload terminated before durable cell disposition; no retry'})
