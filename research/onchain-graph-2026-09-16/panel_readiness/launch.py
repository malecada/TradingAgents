"""Same reviewed 8 GiB/two-CPU/no-elapsed-kill guard, new metadata-only identity."""
import argparse
...
def main():
    ...
    admit(root=root,registration='research/onchain-graph-2026-09-16/panel_readiness/gates.json',
          experiment='eth-panel-readiness-20260916',source=args.source)
    ...
    with (here/'resource.json').open('x') as stream:
        result=launcher.run_with_limit([sys.executable,'-B',str(here/'audit.py'),'--source',args.source],guard,root)
