...
def main():
    ...
    if (root/'research_runs/eth-graph-source-recovery-20260917').exists():
        raise FileExistsError('claimed identity cannot be restarted')
    command = [sys.executable, '-B', str(here/'recovery_launch.py'), '--source', args.source]
    publish(here/'recovery-launch-intent.json', dict(source=args.source, command=command,
    ...
    with (here/'recovery-progress.log').open('x') as output:
    ...
    publish(here/'recovery-started.json', receipt)
