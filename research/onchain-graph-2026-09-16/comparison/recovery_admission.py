"""One explicit, reviewed source-only recovery allowance; no economic amendment."""
import hashlib
import json
...
EXPERIMENT = 'eth-graph-source-recovery-20260917'
PARENT = 'eth-remaining-graph-capture-20260916'
LINEAGE = ['eth-graph-source-20260916', 'eth-graph-prototype-20260916',
           ...
           'eth-matched-input-capture-20260916', PARENT]
...
def target_contract(experiment):
    ...
    value['inputs'] = {k:v for k,v in value['inputs'].items() if k != 'recovery_amendment'}
...
def validate_policy(gate, certificate, baseline, cohort, summary, history, review):
    ...
    expected = dict(schema_version=1, amendment_id='eth-raw-recovery-single-20260917',
                    target_experiment=EXPERIMENT, baseline_experiment=PARENT,
                    baseline_family_sha256=digest(canonical(old_family)),
                    original_family_cap=8, cumulative_prior_claims=8,
                    additional_claims=1, cumulative_cap=9, raw_only=True,
                    automatic_restart=False, stop_on_first_transport_failure=True)
    for key, value in expected.items():
        if certificate.get(key) != value or type(certificate.get(key)) is not type(value):
            raise ValueError('source recovery amendment differs: '+key)
    if old_family['attempt_budget'] != 8 or old_family['prior_attempts'] != 7:
        raise ValueError('original family budget changed')
    if family['prior_attempts'] != 8 or family['attempt_budget'] != 9:
        raise ValueError('recovery cumulative budget differs')
    if family['mechanism_id'] != 'ethereum-missing-source-response-recovery':
        raise ValueError('recovery mechanism changed')
    if certificate['target_contract_sha256'] != target_contract(exp):
        raise ValueError('recovery contract differs from amendment')
    cells = summary['cells']
    source = {r['id'][6:]:r['status'] for r in cells if r['id'].startswith('graph-')}
    if len(source) != 1084 or len(cells) != 1085:
    ...
    if len(missing) != 521 or len(complete) != 563 or cohort['dates'] != missing or cohort['prior_complete_dates'] != complete:
        raise ValueError('recovery does not match exact unavailable cohort')
    if exp['cells'] != ['graph-'+d for d in missing]+['index']:
        raise ValueError('recovery lifecycle cells differ')
    if exp['outputs'] != [c+'.json' for c in exp['cells']]+['summary.json']:
        raise ValueError('recovery outputs differ')
    if set(cohort['prefixes']) - set(missing):
        raise ValueError('reuse prefix outside missing cohort')
    if (cohort['raw_baseline_bytes'] != 54*1024**3 or
        cohort['prior_metadata_baseline_bytes'] != 512*1024**2 or
        cohort['lifecycle_reserve_bytes'] != 128*1024**2 or
        cohort['stop_on_first_transport_failure'] is not True or cohort['automatic_restart'] is not False):
        raise ValueError('recovery storage/stop policy differs')
    ...
    if (review.get('decision') != 'approve-single-source-recovery' or review.get('target_experiment') != EXPERIMENT
            or type(review.get('additional_claims')) is not int or review['additional_claims'] != 1):
        raise ValueError('independent source-recovery approval missing')


def admit_recovery(root, source):
    root = Path(root)
    result = admit(root=root, registration=BASE+'recovery-gates.json',
                   experiment=EXPERIMENT, source=source)
    gate = json.loads((root/(BASE+'recovery-gates.json')).read_bytes())
    exp = gate['experiments'][EXPERIMENT]
    def read(name):
        ref = exp['inputs'][name]
        ...
        if digest(raw) != ref['sha256']:
            raise ValueError('recovery metadata hash differs: '+name)
        return raw
    certificate = json.loads(read('recovery_amendment'))
    baseline_raw = read('baseline_gate')
    cohort_raw = read('cohort')
    history = json.loads(read('history'))
    review = json.loads(read('recovery_approval'))
    validate_policy(gate, certificate, json.loads(baseline_raw), json.loads(cohort_raw),
    ...
    if certificate['approval_sha256'] != digest(read('recovery_approval')):
    ...
    for path in (root/'research_runs').glob('*/claim.json'):
        ...
        if claim['experiment_id'] == EXPERIMENT:
            raise ValueError('recovery identity is already claimed; no restart')
        inputs = claim['experiment'].get('inputs', {})
        ref = inputs.get('recovery_amendment')
        if ref and ref.get('sha256') == exp['inputs']['recovery_amendment']['sha256']:
            raise ValueError('source recovery amendment already consumed')
        ...
        if ref and parent_ref and parent_ref.get('sha256') == certificate['baseline_terminal_sha256']:
            raise ValueError('same source parent already received a recovery allowance')
    return result
