"""Single amended missing-source recovery; immutable predecessor evidence."""
import argparse
...
from recovery_graph import bulk, recovery_day
from recovery_admission import admit_recovery
from recovery_check import check_day
from graph_capture import storage

HERE = Path(__file__).resolve().parent
ROOT = HERE.parents[2]
EXPERIMENT = 'eth-graph-source-recovery-20260917'
REGISTRATION = 'research/onchain-graph-2026-09-16/comparison/recovery-gates.json'
Budget = bulk.Budget
...
def main():
    ...
    with ResearchRun.start(root=ROOT, registration=REGISTRATION,
        ...
        if len(cohort['dates']) != 521:
            raise ValueError('recovery calendar differs')
        ...
        # 512 MiB prior metadata plus 128 MiB dedicated lifecycle/log reserve.
        budget = Budget(existing_raw_bytes=54*1024**3,
                        existing_metadata_bytes=640*1024**2)
        cells, rows = [], []
        for current in cohort['dates']:
            result = recovery_day(current, directory/current, inventory, plan, budget,
            ...
            rows.append(result)
            print(json.dumps(dict(phase='missing_graph_recovery', **result)), flush=True)
