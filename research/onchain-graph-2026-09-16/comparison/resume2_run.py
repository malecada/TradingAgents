"""Single amended missing-source resume2; immutable predecessor evidence."""
import argparse
...
from resume2_check import check_day
from graph_capture import storage

HERE = Path(__file__).resolve().parent
ROOT = HERE.parents[2]
EXPERIMENT = 'eth-graph-source-resume2-20260918'
REGISTRATION = 'research/onchain-graph-2026-09-16/comparison/resume2-gates.json'
Budget = bulk.Budget
...
def main():
    ...
    with ResearchRun.start(root=ROOT, registration=REGISTRATION,
        ...
        if len(cohort['dates']) != 446:
            raise ValueError('resume2 calendar differs')
        ...
        # 1024 MiB prior metadata plus 128 MiB dedicated lifecycle/log reserve.
        budget = Budget(existing_raw_bytes=62*1024**3,
                        existing_metadata_bytes=1152*1024**2)
        cells, rows = [], []
        for current in cohort['dates']:
            ...
            rows.append(result)
            print(json.dumps(dict(phase='missing_graph_resume2', **result)), flush=True)
