"""Single amended missing-source resume; immutable predecessor evidence."""
import argparse
...
from resume_admission import admit_resume
from resume_check import check_day
from graph_capture import storage

HERE = Path(__file__).resolve().parent
ROOT = HERE.parents[2]
EXPERIMENT = 'eth-graph-source-resume-20260917'
REGISTRATION = 'research/onchain-graph-2026-09-16/comparison/resume-gates.json'
Budget = bulk.Budget
...
def main():
    ...
    with ResearchRun.start(root=ROOT, registration=REGISTRATION,
        ...
        if len(cohort['dates']) != 507:
            raise ValueError('resume calendar differs')
        ...
        # 768 MiB prior metadata plus 128 MiB dedicated lifecycle/log reserve.
        budget = Budget(existing_raw_bytes=56*1024**3,
                        existing_metadata_bytes=896*1024**2)
        cells, rows = [], []
        for current in cohort['dates']:
            ...
            rows.append(result)
            print(json.dumps(dict(phase='missing_graph_resume', **result)), flush=True)
