...
def main():
    ...
    if (root/'research_runs/eth-graph-source-resume-20260917').exists():
        raise FileExistsError('claimed identity cannot be restarted')
    command = [sys.executable, '-B', str(here/'resume_launch.py'), '--source', args.source]
    publish(here/'resume-launch-intent.json', dict(source=args.source, command=command,
    ...
    with (here/'resume-progress.log').open('x') as output:
    ...
    publish(here/'resume-started.json', receipt)
