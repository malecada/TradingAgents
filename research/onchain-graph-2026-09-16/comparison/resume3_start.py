...
def main():
    ...
    # Connectivity only: no HTTP request, no data body and no claim on failure.
    with socket.create_connection(('aws-public-blockchain.s3.us-east-2.amazonaws.com', 443), timeout=15) as connection:
        with ssl.create_default_context().wrap_socket(connection, server_hostname='aws-public-blockchain.s3.us-east-2.amazonaws.com') as tls:
            tls_protocol = tls.version()
    admit_resume3(root, args.source)
    if (root/'research_runs/eth-graph-source-resume3-20260918').exists():
        raise FileExistsError('claimed identity cannot be restarted')
    command = [sys.executable, '-B', str(here/'resume3_launch.py'), '--source', args.source]
    publish(here/'resume3-launch-intent.json', dict(source=args.source, command=command,
        root=str(root), tls_preflight=dict(protocol=tls_protocol, http_requests=0), requested_at=datetime.now(timezone.utc).isoformat(), automatic_restart=False))
    ...
    with (here/'resume3-progress.log').open('x') as output:
    ...
    publish(here/'resume3-started.json', receipt)
