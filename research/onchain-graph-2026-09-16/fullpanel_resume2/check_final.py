...
PREDECESSOR = 'eth-full-history-feature-panel-resume-20260922'
MAX_BUCKET = 256 * 2**20
...
def verify_plans(root, resume, original):
    ...
    assert resume['excluded_empty_hash_root'] == '/home/malecada/master_thesis/onchain-fullpanel-resume-hash-scratch-20260922'
    assert not list(Path(resume['excluded_empty_hash_root']).iterdir())
...
def compute_resource(root, source):
    ...
    assert value['memory_high_bytes'] == 6 * 2**30 and value['memory_swap_max_bytes'] == 2**29
    ...
    if clean:
        ...
        assert value['kernel_controls'] == {'memory.max':str(6*2**30), 'memory.high':str(6*2**30), 'memory.swap.max':str(2**29)}
...
def main():
    ...
    assert claim['family']['prior_attempts'] == 15 and claim['family']['attempt_budget'] == 16
    ...
    required_sources.update(BASE+'/'+p for p in ('check_final.py','run.py','launch.py','memory_guard.py'))
    required_sources.update('research/onchain-graph-2026-09-16/fullpanel_resume/'+p for p in ('run.py','hash_union.py'))
