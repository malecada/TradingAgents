"""One cumulative15/15 continuation; original206completed days are immutable."""
import hashlib
import json
...
def validate(gate,plan,amendment,history,approval):
    ...
    if f['prior_attempts']!=14 or f['attempt_budget']!=15 or f['mechanism_id']!='ethereum-full-history-daily-feature-extraction-resume':raise ValueError('cumulative continuation budget differs')
    ...
    if plan['hash_roots'][0]['path']!='/home/malecada/master_thesis/onchain-fullpanel-hash-scratch-20260922' or plan['hash_roots'][1]['path']!='/home/malecada/master_thesis/onchain-fullpanel-resume-hash-scratch-20260922':raise ValueError('scratch ownership differs')
    if plan['memory']!=dict(memory_max_bytes=6*2**30,memory_high_bytes=4*2**30,memory_swap_max_bytes=512*2**20,reserve_bytes=3*2**30,start_reserve_bytes=9*2**30):raise ValueError('shared-host protection differs')
    if amendment!=dict(schema_version=1,target_experiment=EXPERIMENT,prior_claims=14,additional_claims=1,cumulative_cap=15,target_contract_sha256=contract(e),network_allowed=False,prices_or_models_allowed=False):raise ValueError('single continuation amendment differs')
    failed={'eth-temporal-motifs-20260916','eth-seven-day-pilot-20260916',PRIOR}
...
def admit_resume(root,source,*,review=False):
    ...
    validate(a.spec,read('plan'),read('amendment'),history,approval)
