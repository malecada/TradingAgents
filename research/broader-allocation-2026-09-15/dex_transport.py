"""Bounded read-only RPC transport derived from frozen dated_mark_transport.py."""
import json
import http.client
import signal
import urllib.error
...
def public_post(url, payload):
    ...
    """
    if url not in {'https://ethereum-rpc.publicnode.com', 'https://mainnet.base.org', 'https://arb1.arbitrum.io/rpc'}:
        raise ValueError('unregistered public endpoint')
    if not isinstance(payload, dict) or payload.get('jsonrpc') != '2.0' or payload.get('method') not in {'eth_chainId', 'eth_getBlockByNumber', 'eth_call', 'eth_gasPrice', 'eth_getCode'}:
    ...
    try:
        ...
        request = urllib.request.Request(url, data=encoded, method="POST",
            headers={"Accept": "application/json", "Content-Type": "application/json",
                     "User-Agent": "RegisteredDexSourceResearch/1.0"})
        try:
