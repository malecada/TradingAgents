"""Exact600second/512MiB/twoCPU wrapper; no modification to the existing guard."""
import argparse
...
def launch(command):
    return guard.run_guard(command, wall_seconds=600, rss_limit_bytes=512*1024**2)


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--source', required=True)
    parser.add_argument('--recipe', choices=['H2','H3','H4','H5'], required=True)
    parser.add_argument('--report', required=True)
    args = parser.parse_args()
    with open(args.report, 'x') as stream:
        result = launch([sys.executable, '-B', str(HERE/'conditional_run.py'), '--source', args.source, '--recipe', args.recipe])
