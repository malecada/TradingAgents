...
## Section 44: Meta-Labeled Trend System — Pre-Registered Negative (2026-07-15/16)

**Question.** Can a prediction model be resurrected by changing the target: instead of forecasting price direction (repeatedly negative — §12, §40, BT8/BT11), predict the *conditional success probability* of an already-positive model-free trend signal (Lopez de Prado meta-labeling), using exogenous positioning/on-chain covariates?

**Motivation.** A 101-agent adversarially-verified deep-research pass (2026-07-15) reconfirmed the external consensus: no credible net-of-costs edge exists in direct crypto price forecasting 2022–2026 (foundation models 50–53% DA, degraded by fine-tuning; PatchTST/iTransformer negative IC on a 66-coin daily cross-section; 3–10 bps of costs flip every published gross-positive result; the one net-positive claim, CryptoGAT, refuted 0–3 at verification). Two doors were left open: exogenous covariates and alternative targets (meta-labeling / triple-barrier — unexplored, not refuted). This experiment tested both simultaneously.

**Design (pre-registered before any run; `experiments/metalabel/`, `experiments/metalabel_v2/`).** Frozen trend primary (MA-cross 5/20, 10/40, 20/60 + Donchian 20/10 vote ensemble, long-flat, 8-coin live universe); triple-barrier labels (PT 2.0σ / SL 1.5σ / 15-bar vertical, SL-first intrabar, entry at next open); 14 PIT exogenous features (Coinglass funding/OI/liquidation/smart-money/taker + CM on-chain z-scores + F&G + cross-coin breadth) + price/trend context; purged expanding walk-forward (21-day embargo); LightGBM vs logistic and constant-base-rate baselines, isotonic calibration, purged inner CV. Gates: G1 (AUC 95% CI excludes 0.5 + beats baselines) → G2 (do-no-harm ΔSR, p_pos ≥ 0.90) → G3 (locked holdout 2025-04-01…2026-06-30, one-shot). Trial ledger + freeze contract per §39–41 house standard.

**v1 (entry events only): G1 FAIL, underpowered.** 203 pooled events (8 coins × 3.75 yr — trend entries are rare, ≈7/coin/yr), min-150-train fold skips left only 44 OOS events. LGB AUC 0.347, iid CI [0.18, 0.52]. Uninformative beyond "no evidence of skill."

**v2 (re-registered 2026-07-16: dense in-bar events, dev from 2022-01, min-train 75, cluster coin-month bootstrap): G1 FAIL, well-powered.** 3,177 dense events, **2,604 OOS predictions**. LGB AUC **0.481**, cluster-bootstrap 95% CI **[0.446, 0.522]**; logistic 0.494; LGB Brier 0.282 *worse* than constant-base-rate 0.252. The CI excludes any meaningfully useful accuracy (AUC > 0.52 rejected at 2.5%). chosen_tau = None; G2/G3 never ran; **holdout remains unspent**.

**Verdict.** Trade-outcome predictability, conditional on a trend entry, from positioning + on-chain + sentiment covariates at daily frequency is **indistinguishable from zero and bounded tightly around chance**. The meta-labeling door — the last open alternative-target route from the deep-research pass — is now closed with adequate power, matching every prior directional-prediction negative (§12, §33, §40, §43). The negative is methodologically clean: pre-registered gates, frozen parameters, purged evaluation, ledgered trials, holdout untouched.

**Engineering notes for reuse.** (1) The stack (`tradingagents/metalabel/`: frozen primary, leak-tested triple-barrier labeler with uniqueness weights, PIT feature assembly, purged WF splitter, cluster-bootstrap G1 evaluator, meta-filtered replay) is fully reusable for any future event-based experiment. (2) Final-review catch worth institutionalizing: tz-aware PIT-store index reindexed against tz-naive event dates silently returned all-NaN for every exogenous feature — a zero-coverage RuntimeError guard now refuses to run gates on a gutted feature matrix; regression-test every tz-crossing boundary in PIT joins. (3) Coinglass/on-chain coverage at event dates is only 27–40% even post-2022 (store gaps on satellites); any future covariate experiment should quantify feature coverage before registration.
