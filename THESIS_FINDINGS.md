...
### 23.12 — Sentiment analyst v3 A/B (assignment §4.6 / TODO-P0-2)

**Context**: §11/§12 PIT sentiment showed GDELT hurts BTC; §23.11 LOO confirmed sentiment = noise at modulator level. Per `sentiment_analysis_report.md` (research review) the legacy free-text Reddit+GNews "vibe" analyst is the empirically-worst-case design. Spec [`docs/superpowers/specs/2026-05-23-sentiment-analyst-v3-design.md`] adopts Option C from the report: deterministic `SentimentSnapshot` (Pydantic) built from CryptoBERT + FinBERT polarity, Liu-Tsyvinski Google-Trends attention, F&G 24-week-EMA regime gate, and GDELT-themed event flags, feeding both a narrow ticker-anonymized LLM event extractor (Self-MoA N=5 on `gpt-4o-mini`) and the modulator agent's prompt as numeric features.

**Branch**: `feature/sentiment-analyst-v3` (off `feature/hybrid-modulator`), 24 commits, 266/270 tests green. Default `sentiment_mode=legacy` preserved until validation passes.

**Setup**: 4-variant A/B over BTC + ETH, 2026-01-16 → 2026-04-15 (90 bars, same window as §23.11). Variants:
- **A — pure_quant**: no sentiment analyst (`market, onchain, prediction` only).
- **B — legacy_sentiment**: current production free-text analyst (Reddit + GNews + Alpha Vantage + global news).
- **C — v3_features_only**: structured SentimentSnapshot fed to modulator + agent state, narrow LLM event analyst BYPASSED (`--sentiment-skip-llm`).
- **D — v3_full**: structured snapshot + narrow LLM event analyst.

All variants: V5 quant routing (`v5_2coin` preset), all-`gpt-4o-mini`, replay cache enabled. Hetzner CX22 (4 GB RAM, 2 cores) sequential. ~40h wall-clock total.

#### Results (point estimate SR, 90 bars)

| Variant | BTC SR | ETH SR | avg | trades (BTC/ETH) |
|---|---|---|---|---|
| A_pure_quant | 7.58 | 5.28 | 6.43 | 81 / 70 |
| B_legacy_sentiment | 7.28 | 5.65 | 6.47 | 81 / 70 |
| C_v3_features_only | 6.59 | 4.66 | 5.63 | 80 / 64 |
| D_v3_full | 7.88 | 4.28 | 6.08 | 82 / 67 |

#### Paired bootstrap (10,000 resamples, daily PnL, ANN=365)

| Comparison | Coin | ΔSR | 95% CI | P(Δ > 0) | Significant |
|---|---|---|---|---|---|
| D vs A (full v3 vs pure quant) | BTC | +0.15 | [−0.92, +1.11] | 0.624 | no |
| | ETH | **−1.02** | [−2.28, +0.13] | 0.044 | **yes (hurts)** |
| D vs B (full v3 vs legacy) | BTC | +0.27 | [−1.37, +1.70] | 0.631 | no |
| | ETH | **−1.39** | [−2.56, −0.41] | 0.001 | **yes (hurts)** |
| D vs C (LLM analyst increment) | BTC | +0.97 | [−0.94, +2.13] | 0.879 | no (marginal positive) |
| | ETH | −0.43 | [−1.49, +0.53] | 0.196 | no |
| C vs A | BTC | −0.82 | [−2.32, +0.50] | 0.143 | no |
| | ETH | −0.60 | [−1.62, +0.42] | 0.121 | no |
| C vs B | BTC | −0.70 | [−2.00, +0.41] | 0.106 | no |
| | ETH | **−0.97** | [−2.07, +0.04] | 0.031 | **yes (hurts)** |
| B vs A (legacy ≈ no-sentiment) | BTC | −0.12 | [−1.95, +1.81] | 0.425 | no |
| | ETH | +0.37 | [−0.41, +1.21] | 0.815 | no |

#### Findings

1. **v3 architecture hurts ETH at significance**. Both C (structured-only) and D (full v3) significantly underperform the legacy and pure-quant baselines on ETH (P = 0.031 and P = 0.001 respectively vs B; P = 0.044 vs A for D). The Liu-Tsyvinski attention proxies + F&G + GDELT events injected into the ETH modulator prompt actively bias position sizing in the wrong direction over this window.

2. **v3 is BTC-neutral at the variant level but the narrow LLM event analyst materially helps**. D vs C on BTC = +0.97 SR (P = 0.879, marginally above 1σ). The structured snapshot alone (C) underperforms; the narrow LLM that interprets the snapshot as event-driven narrative is where the BTC alpha lives. Compare to §23.11 LOO finding that BTC sentiment contribution was noise on a non-narrow free-text analyst — the narrow scope (events only, anonymized) recovers a positive contribution.

3. **Legacy sentiment ≈ no sentiment on both coins**. B vs A is statistically indistinguishable from zero on BTC (P = 0.425) and ETH (P = 0.815). The free-text Reddit/GNews analyst neither helps nor hurts on this window — wasting LLM budget for zero signal, consistent with §23.11 LOO P-values ≈ 0.5.

4. **Per-coin asymmetry confirmed (again)**. BTC and ETH need different sentiment policies, mirroring the §20 V5 quant routing and §23.11 analyst LOO findings. ETH consistently treats added LLM-modulator features as noise that biases sizing; BTC has marginally interpretable sentiment when scope is narrow.

5. **Modulator-only alpha is the real story**. A_pure_quant (no sentiment analyst at all, modulator LLM still active) hits portfolio SR 6.43 over 90 bars — caveat: small-sample inflation (SR > 5 across all variants suggests the 90-bar slice happens to be a tailwind regime; full-1-year window from §23 sees ETH hybrid SR 4.68 vs pure V5 3.58, more realistic delta of +1.10).

#### Production recommendation (per acceptance criteria from spec)

Acceptance per-coin: ΔSR ≥ +0.10 vs A AND ≥ +0.20 vs B, plus D-C ≥ +0.05.

- **C fails both bars on both coins.** Drop.
- **D fails on ETH (significant negative).** BTC passes the "no harm" bar (P = 0.624, no statistical alpha but no significant loss) and shows promising +0.97 over C.
- **B vs A is null**: no point keeping legacy sentiment.

**Ship matrix** (asset-specific routing, mirroring §20 / §23.11):
- **BTC**: ship D (full v3) — narrow LLM event analyst adds marginal SR with no significant downside. Larger-sample re-test recommended before final commit.
- **ETH**: drop crypto_sentiment analyst entirely (configure `selected_analysts` without it for ETH propagate). Saves ~25 % LLM gen time per ETH bar with statistically-significant SR improvement.

This per-coin routing is a Phase-2 follow-up requiring a TradingAgentsGraph configuration touch (analyst set is currently global, not per-coin); the v3 sentiment code itself is production-ready behind `sentiment_mode=v3` flag.

#### Caveats

1. **Small sample (90 bars)**. All SR values > 3 inflated by tailwind regime. Effect sizes (deltas) should be re-validated on the 1-year window per §23.
2. **`A_pure_quant` modulator agent still runs** — the Layer-2 LLM modulator (factual/subjective + Self-MoA) is in all variants; this isolates the *crypto_sentiment analyst contribution* but not the modulator's own.
3. **Replay cache cascade**: per `feedback_replay_cache_model_variation`, A's market/onchain/prediction analyst outputs do not propagate to B/C/D cleanly due to BM25 memory cascading variant differences across the LLM team. Each variant paid full LLM cost. Total spend ≈ $40 in `gpt-4o-mini`.
4. **C_BTC first pass produced 99 error rows** before the `'HumanMessage' object has no attribute 'tool_calls'` bug fix (`f910855`). The skip-LLM path returned `messages=[]` leaving the prior HumanMessage as state's last — fixed by emitting an empty-content `AIMessage` stub. Re-ran cleanly.
5. **Bootstrap CI computed on daily `hybrid_ret` PnL** (`backtest/daily_returns.csv`), ANN=365. Resampling preserves serial autocorrelation only via paired-by-date construction, not block bootstrap.

#### Data artifacts

| File | Description |
|---|---|
| `scripts/run_sentiment_v3_ab.py` | 4-variant harness |
| `scripts/agg_v3_results.py` | Point-estimate aggregator |
| `data/sentiment_v3_ab/{A,B,C,D}_*/{bitcoin,ethereum}_2026-01-16_2026-04-15.csv` | Signal CSVs (90 rows × variant × coin) |
| `data/sentiment_v3_ab/{A,B,C,D}_*/backtest_{bitcoin,ethereum}/{summary.json,daily_returns.csv,hybrid_vs_baseline_equity.png}` | Per-coin per-variant backtest output |
