...
## Section 88: September 9 system repairs and bounded evidence correction — no strategy graduation (2026-09-09)

This addendum takes precedence over incompatible claims in earlier sections. The audited accounting, forecast availability, causal data, execution state and evidence registration defects have been repaired in the isolated `fix/system-audit-2026-09-09` branch. Original data, worktrees and results remain preserved. Legacy NLST rank claims, positive S1/champion/Bybit performance and broad no-edge conclusions remain withdrawn or qualified by `docs/audit/corrections.jsonl`. The repair does not reopen closed research families or spent holdouts.

The preregistered `audit_correction_2026_09_09` run completed once, using the existing 2021-01-01 through 2025-03-31 development interval. Seven S2/S3 cells and sixteen saved ENet forecast diagnostics are reported in full below. No holdout was evaluated, no forecasts were refitted and no parameters were selected.
...
Reproduction: source commit `cc6801e81e25f05cdfbae77d560b59fa43e16dd9`; committed gate SHA256 `611d12466b06865e38cb6fce51f9ce30713d7ac472a0effe02686c26b9b82bd5`; pre-run correction-policy SHA256 `23cdf85b45138a5a0429e19bf20fd195402bbc9aec4e03cdb28e8eb2ed8d8350`. The immutable result is `data/predlab/audit_correction_2026_09_09/result.json`, SHA256 `21be37f6c53d9827a150309fdf722953846bf88a87d72c1a2f07b38872115a3c`. It records input hashes, 23 output hashes, all denominators and unchanged-input verification. The frozen gate still records its pre-run status; actual completion is appended in the ledger and corrections register. No original gate criteria were edited after results.

Engineering verification passed 779 offline tests, with two historical parity/marker tests deselected and one three-test physical-store coverage file excluded because raw stores remain in the original worktree. Synthetic regression and independent review reports are under `docs/audit/`. The repaired S1 execution and paper measurement code has not been deployed to or verified on the VPS. Zero validated strategies remain; rejection of these fixed constructions does not establish that no crypto strategy can have an edge.

## Section 89: September 9 fixed-lead reevaluation — measured failures, unavailable books and a qualified ranking result (2026-09-09)

The separately preregistered `audit_reevaluation_2026_09_09` cycle completed all 26 fixed cases using reviewed source `49fb9e47c4d9e40b9c3f1b7de475041ec63aac76`. This addendum takes precedence over incompatible earlier claims about the affected measurements. The original gates, raw stores and artifacts are preserved; the correction does not restore a virgin holdout, select a new strategy, or repeat the earlier S2/S3/ENet correction. The complete report, cell-level evidence, triage and independent reviews are under `docs/reevaluation/`.

**Twenty-one accounting cases are unavailable, not confirmed negative results.** All 12 momentum cases encounter missing held marks at BZRXUSDT or BNXUSDT contract tails, and all six carry cases encounter the LUNAUSDT price tail. Nearby hourly data contain stopped trading or frozen zero-volume bars, while funding may continue. Neither the last observed close nor a successor ticker establishes the correct settlement cashflow. A trustworthy replay requires verified closure, settlement and funding treatment. The momentum benchmark is also unavailable, but every candidate book was attempted independently.

Three liquidation-fade cases encounter internal hourly gaps in TRX/FIL/LTC. All three source files omit February 26–28 and April 1–2, 2022 despite active daily bars and hourly bars on either side; month-level availability metadata did not flag the gaps. Daily bars and short alternative-cache row counts do not reconstruct the hourly path. These three cases remain unavailable. A first blocking observation is not an exhaustive inventory of all future required data.

The other three liquidation-fade cases retain all 1,551 daily observations and fail both original primary requirements, SR≥1 and DSR≥0.9. Their corrected SR / original-n=100 DSR / current-n=150 DSR values are: threshold 2.5/6h, **0.165715 / 0.014308 / 0.009950**; threshold 3.5/6h, **0.815429 / 0.305976 / 0.258884**; threshold 3.5/24h, **0.981334 / 0.233004 / 0.192562**. Zero transaction fees still yield original-denominator DSRs only 0.105815, 0.546222 and 0.292796. The preliminary P0/P1/P2 conjunction passes at family level, with only three fully scoreable P2 cells. The original funding exclusion remains a qualification. The invalid log-PnL diagnostic is either explicitly rejected as inadmissible during the May 2022 crash or, for threshold 3.5/6h, gives SR−0.129438. Expensive placebo performance tests are not run after conclusive primary failures; their RNG streams are advanced as registered.

PRX's corrected Engle–Granger selector fails the original conditional persistence comparison: mean monthly selected persistence **10.9611%** versus random **11.3000%**, ratio **0.970009** versus the required 1.5, and one-sided paired Wilcoxon p **0.812785** versus <0.05. All 50 months remain present; six unavailable selected-pair outcomes involving TOMO/RNDR/MATIC leave 47 complete months. The registered dependence sensitivity is unavailable; no reduced-calendar substitute or P1 trading test was executed. The result does not reject all pair-trading constructions.

NLST4 retains **3,981 pools: 1,205 prior-history and 2,776 new-set**, with all 2,776 new-set returns available. There are **2,718 scoreable new-set pools and 58 unavailable scores**, across all 17 quarters. The corrected composite passes descriptive T1: IC **0.142362**, quarter-bootstrap fifth percentile **0.083220**, with all 1,000 draws finite. The original qualified artifact used 2,770 scoreable pools and reported IC 0.091910; this is not a paired comparison on an identical sample or an attribution to a particular repair. Ownership is unavailable throughout, no new-set deployer-performance observation is identifiable, and only 14 new-set deployer counts are identifiable; the ranking result does not validate all ten registered component features.

The 544-pool retrospective top quintile fails economic T2: mean seven-day return **+19.0978%**, one-sided NW p **0.228018**, mean excluding the largest absolute return **+0.0442%**, median **−64.9484%**, largest absolute-return share **13.8972%**, and mean at $5,000 stress **−21.4164%**. The positive ranking association is retained on the scoreable historical cohort; the economic gate fails significance and the stress condition. The global q80 selector remains retrospective, the outcomes were previously examined, and no causal online entry policy or fresh out-of-sample confirmation is established. Original gas/LP assumptions exclude MEV and are not live fills.

Verification: 171 additional integrated offline tests passed before execution; independent reviews checked all recorded input/output hashes, saved arithmetic, cohort chronology and ledger consistency. The append-only ledger retains its 334,207 prior bytes and adds exactly 26 rows with 26 unique trial IDs; all 31 earlier predlab registrations remain unchanged. The manifest identifies 13,217 distinct input paths, ten ancillary outputs and five immutable result hashes. Gate SHA256 `b74efa4273a38339cab652cb140b0aa64df1778a1f7f8bcb015e49c3f357e894`; pre-run correction-policy SHA256 `fba631d9bb458decc4b278c1b224a2441d97b75d7a328dffa0850c9799619a4d`. Completion and qualifications are appended afterward to `docs/audit/corrections.jsonl`.

Zero validated strategies remain. No measured gate reversal was established, while the 21 unavailable accounting cases cannot adjudicate their earlier rejection under the repaired contract. The immediate recovery requirements are instrument settlement records and genuine internal bar coverage. A subsequent empirical correction requires its own committed registration, preserving sample membership and prior holdout spending. The existing thesis already withdraws the superseded strategy/causal-validation claims; this cycle does not modify the manuscript or VPS.

## Section 90: September 10 market recovery and guarded replay — two conditional failures, 22 unavailable accounting cases (2026-09-10)

This addendum supersedes Section 89's count of three complete liquidation-fade failures and 21 unavailable accounting cases. The new recovery, exposure-only diagnostic and financial replay were separately registered and committed before their respective outputs. The complete evidence and reviews are in `docs/data-recovery/DATA_RECOVERY_2026-09-10.md` and its linked manifest. No strategy is validated or promoted.

The timestamp-only inventory identifies 6,832 internal missing symbol-hours across 49 of the 217 frozen hourly inputs. All **5,568 ordinary missing hours across 47 symbols are recovered**, including the originally identified 360 TRX/FIL/LTC hours. Each recovered hour is absent from the current checksum-valid monthly archive but present in an official daily archive and independently agreeing public API response; observed overlaps also agree exactly. The local downloader's any-row-is-complete monthly predicate compounded the provider's incomplete monthly files. Coverage controls now require an intact clock, expose gaps and retry observed partial months despite stale archive-absence markers. The dedicated recovery path uses verified daily/API observations because retrying monthly files alone cannot repair them. Original files and in-window values remain unchanged.

The other 1,264 hours remain quarantined: 638 BNX and 626 ICP. Official evidence establishes old/new perpetual contract breaks in February 2023 and June/September 2022 respectively. A reused ticker or token-conversion ratio does not establish contract continuity. The daily manifest has no internal clock gaps among its 442 files with development observations, but complete dates do not establish correct terminal cashflows.

Official notices establish BZRX, LUNA and BNX closure times; exact historical settlement cashflows, fees and final funding remain unestablished. Archived official FAQ text describes a one-hour index average, while the current text describes 30 minutes, so a current formula cannot be backprojected without evidence. Minute candles, the last trade close and funding records after closure are insufficient. The original TOMO/RNDR/MATIC contracts also terminate before six PRX persistence outcomes can acquire the required 25 daily observations. VIC/RENDER/POL are separately launched futures; relabeling them cannot complete the old fixed-beta ADF outcomes. No PRX statistic is rerun, and all 50 months remain in its denominator.

The original-input exposure diagnostic retains six cells × five registered closure events. Four liquidation-fade schedules hold a 10% LUNA target during the May 12, 2022 15:00–16:00 UTC interval containing its 15:30 closure. **Threshold 3.5 / 24h, previously reported as a complete failure at SR 0.981334, is withdrawn as an executable full-window measurement.** The three other conflicted cells were already unavailable from earlier gaps. The dedicated six-record forensic ledger contains no strategy performance statistics and is excluded from financial trial counts.

The conditional financial replay then reconstructs every schedule using the recovered prices, under source `a87b67b97cd88ed3bbe83e2dd03eca99a5082d33`. All six original cells are retained, and guards reject unknown terminal exposure before primary, cost, convention and actual placebo books. Both H6 cells retain all 1,551 development days and fail both original SR≥1 and DSR≥0.9 requirements:

| Threshold / horizon | Net SR | Maximum drawdown | Original DSR (100) | Current DSR (156) | Status |
|---|---:|---:|---:|---:|---|
| 2.5 / 6h | 0.159713 | 71.21% | 0.013861 | 0.009289 | Conditional measured FAIL |
| 3.5 / 6h | 0.815672 | 16.59% | 0.306207 | 0.254849 | Conditional measured FAIL |
| 2.5 / 24h, 2.5 / 48h, 3.5 / 24h, 3.5 / 48h | unavailable | unavailable | unavailable | unavailable | Unverified LUNA terminal exposure |

The original cost, funding exclusion and five-event lifecycle scope qualify the measurements. The two H6 SRs differ only slightly from September 9's 0.165715 and 0.815429. Even at zero fees, original-denominator DSR remains 0.103181 and 0.546471. The original P0/P1/P2 conjunction passes: both H6 P2 cells are fully scoreable, with 5,069 and 709 trigger windows and means 64.8364bp and 107.8974bp. Longer-horizon P2 cells retain all windows but have null gate values because 4/19 and 1/11 windows respectively require unknown terminal valuation. Their observed subsets cannot pass. Expensive placebo performance tests are skipped after primary failure or data unavailability, with the registered RNG advancement and no replacement draws. No new placebo p-value is claimed.

The current 24-cell accounting interpretation is **two conditional measured failures and 22 unavailable cases: 12 momentum, six carry and four liquidation-fade**. This evidence does not adjudicate the unavailable strategies' economic signs. The PRX conditional failure and qualified NLST4 ranking pass/economic failure from Section 89 remain unchanged. No model was refitted, no strategy was selected, no holdout was evaluated and no manuscript or VPS change was made.

Verification passed 173 integrated offline tests before execution. Independent artifact reviews reconcile 272 replay inputs, three ancillary outputs, seven saved return streams and exactly six unique financial ledger appends after the unchanged 411,461-byte prefix. All old gates and outcomes remain preserved. Replay result SHA256: `2a210cb47723857e2b6b8e318f174e3becd010a91bbcf9405b47fec26d7ebd66`; gate: `987ecb256dcd430f7c5fce0d101cf1f5f020d1c671e8465eddcf86ce9bc9ddbf`; pre-run correction policy: `6bae0081d6d114686d3ac807bb7172dbbbc63f13ac1bb07b0917bf20533cd312`. Completion records are appended after outcomes, without editing gate criteria. Further financial replay requires verified historical terminal accounting and a new committed registration. Zero validated strategies remain.

## Section 91: Historical settlement evidence follow-up — default rules recovered, exact cashflows unavailable (2026-09-10)

The separately registered `audit_settlement_evidence_2026_09_10` investigation completed public documentary and data-integrity checks for BZRXUSDT, LUNAUSDT and the February 2023 BNXUSDT incarnation. Registration commit `506373efe210ed06bb6a1c1a48a8ba26ff382bb1` preceded new archived evidence. Full case matrices, source receipts and the unsent provider request are under `docs/settlement-evidence/`; the canonical report is `SETTLEMENT_EVIDENCE_2026-09-10.md`.

A November 4, 2024 Binance notice explicitly makes the 30-minute / 1,800-index-observation settlement rule effective November 11, 2024 at 08:00 UTC, including delisted contracts. This establishes the historical default for BNX's March 2025 event more directly than the current 2026 FAQ. Older captures support the one-hour default before BZRX and LUNA. BZRX's notice captured one day after closure has a body identical to the preserved current notice. Historical fee and funding FAQs establish ordinary fee examples and mark-price × quantity × rate funding; they do not establish every event's automatic-settlement fee or exact terminal cashflow. A quarterly-delivery settlement fee cannot be imported into perpetual delisting.

Neither public minute index candles nor a current estimated-settlement field supplies an exact old settlement price. The genuine LUNA and BNX event-day archives pass checksums, and 60/31 requested API rows respectively match all twelve archived fields. Conditional BNX candle envelopes remain data diagnostics rather than admitted bounds on actual settlement. A complete historically applicable rule with authentic required observations, or a verified exact terminal mark with the required event mapping and precision, could establish a value without an account receipt; such evidence remains incomplete.

Ordinary income-history retention does not establish asynchronous-export retention. Current export documentation specifies a maximum one-year requested interval but no earliest retained date. Relevant old exports or provider records therefore remain possible but untested sources. No account access or outgoing provider message occurred; the precise evidence request is prepared but unsent.

The cycle retains all **52 direct requests / 2,885,220 response-body bytes**, including 46 HTTP-200 responses, two empty HTTP-202 responses and four timeouts. Parent verification confirms all 2,377 prior recovery/replay artifacts, 222 original input hashes, earlier gate objects and six relevant source files remain unchanged. The 428,150-byte financial ledger has no new rows. No financial replay, model fit, holdout, manuscript edit or VPS change occurred. **Two conditional measured failures and 22 unavailable accounting cases remain; zero validated strategies.** The completed evidence gate cannot be reused for an empirical replay.

## Section 92: Settlement and funding event accounting — synthetic engineering validation (2026-09-10)

The authorized accounting extension adds `tradingagents/event_accounting.py` under the committed design `3ef9f3b`, independently of the frozen return-based engine. Signed contract quantities are marked with linear price differences and explicit multipliers; settlement applies to incoming quantity, charges its declared fee once and permanently closes the original contract. Funding uses held quantity and its own event mark/rate under a separately declared complete calendar. Explicit simultaneous-event sequence and boundary policy prevent a same-time flat target from bypassing settlement. Missing required evidence remains unavailable. Calendar completeness and provenance authenticity remain external admission requirements.

Verification passed **61 synthetic tests and 173 existing accounting/lifecycle/recovery regressions, 234 in total**. Independent reviews found and reproduced numerical lot-flooring and reduce-only maintenance defects during implementation; regression-tested fixes use Decimal input sizing and an explicit eight-machine-epsilon relative policy for floating NAV drift. The independent final numeric review found no unresolved prior finding. Hand-calculated checks cover signed terminal PnL, cash rounding, funding around closure, incoming quantities, fee attribution, successors, missing observations and complete reporting clocks. The report, design, test logs and reviews are in `docs/event-accounting/`.

All 2,476 prior local recovery/replay/settlement artifacts and 222 referenced original input hashes pass preservation checks. The prior scripts, bar engine, xsect wrappers, gates and results are unchanged; the financial ledger remains 428,150 bytes with SHA-256 `710a4f087325bfb408ed8d051963a473e8d8a47b445ffae0238c565aff6dc791` and no new rows. This was not a financial trial. Legacy carry retains its documented daily funding approximation, while momentum and liquidation-fade retain funding exclusions. A future event-based empirical adapter requires admitted data, explicit remaining execution/capital-charge conventions and a fresh committed registration.

The provider request remains unsent. Official visitor support chat was located, but an interactive browser/connector and contact details were unavailable. No historical terminal price or fee was filled, no strategy or holdout was rerun, and no main merge, manuscript edit or VPS verification/deployment occurred. **Two conditional measured failures and 22 unavailable accounting cases remain; zero validated strategies.** Synthetic correctness does not establish venue-specific historical cashflows or production margin behavior.

## Section 93: Operational integration and read-only VPS audit (2026-09-10)

The user deferred missing historical settlement records and ended the Binance support route. The research request was never sent; a supplied contact address had been entered before visitor security verification. This corrects Section 92's earlier browser/contact limitation without changing the historical evidence result. No alternate terminal price, fee, shortened cohort or relaxed gate was substituted. **Two conditional measured failures and 22 unavailable accounting cases remain; zero validated strategies.**

Read-only SSH inspection at 10:23:56 UTC found the VPS paper/execution checkout at `558ffe7456dbfa95e89f960df713a4e3d8125ea0`, monitor checkout at `ed5e22f63d9e2d8ab4bf163103f40b0029eb0e7b`, no v2 journals and no funding store. Legacy paper journals contain 39 VT10 and 38 champion rows; the testnet journal contains 17 rows. These file counts and dates do not establish net performance or reconciled positions. Monitor, Caddy and cron services are running; legacy quant/hybrid timers are inactive. No account API, secret contents, full cron body, remote mutation or order was accessed or performed.

Local integration merges current main into the reviewed corrections while preserving both histories and existing worktrees. The S1 monitor now requires version-2 net measurements, explicit state consistency and an intact reporting clock; base and already-net overlay returns remain separate, with no second scaling. Initial state and observed flat warmup are distinguished from unavailable data. Volatility scale eligibility uses 20 contiguous base net returns. The frontend preserves null values, reports reconciliation and labels account equity changes as cashflow-unadjusted. Invalidated S1 references and its derived threshold/countdown are withdrawn from current views while historical gate files remain unchanged. The backup preserves four paper journal files through partial rollout and failed synchronization, without including operational logs.

Verification passed **489 integrated Python tests and 25 frontend tests**, plus the frontend build, lint and synthetic browser checks. Independent reviews reproduced and resolved schema, null-value, warmup-count and reconciliation-label defects. Build output retains a non-blocking bundle-size warning; Python dependencies emit five deprecation warnings. All 2,476 previous recovery/replay/settlement artifacts and 222 original input hashes pass preservation checks. Accounting engines, paper/live writers, original stores, gates and results remain unchanged; the 428,150-byte financial ledger retains SHA-256 `710a4f087325bfb408ed8d051963a473e8d8a47b445ffae0238c565aff6dc791` with no new rows.

The report and evidence are under `docs/operations/`. A minimal manual monitor rollout/rollback sequence avoids the generic deployment script, which enables unrelated stopped trading timers, and preserves data roots and execution state. No deployment occurred. Missing funding must be addressed before any corrected paper chain is started; adding funding after a broken measurement does not automatically repair continuity. The standalone event-accounting module remains outside S1, whose funding treatment retains its daily approximation. This stage establishes local compatibility and operational findings, not production readiness or strategy validation.

## Section 94: Current public funding capture and paired paper admission (2026-09-10)

Current public funding infrastructure and an explicit snapshot reader are implemented on source `7d2a0ae9ef4ad707774360d2f67af40258a7d2e3`. The collector preserves raw responses, request outcomes, published event times/rates/marks, original contract identities and acquisition provenance in new immutable directories. It does not change the historical funding store. A public pilot exposed millisecond timestamp jitter; the separately committed `7654889` amendment permits derived comparison labels within one second after an exact UTC hour while preserving raw daily membership. A second integrity check identified five Chinese-symbol contracts omitted by ASCII-only validation; the correction preserves exact Unicode provider identities without aliases or normalization.

The final current-universe capture includes **528/528 eligible contracts, 24,506 Regular funding events and 532 HTTP-200 requests**, with zero omitted market-eligible identities. For September 9, **525/528 contracts pass the qualified funding-coverage check**. SKRUSDT, TUSDT and ZKCUSDT retain unavailable days: observed four-hour grids and genuine following boundaries conflict with the prior seven-day hourly minimum-spacing inference. Current schedule metadata cannot prove the past effective schedule; the rule remains unchanged. Earlier three-name and restricted 523-name captures are retained. No economic result is inferred from query exhaustion or coverage.

With an explicit snapshot configured, both paper algorithms prepare their rows before either append. Missing relevant funding, closing prices, required current quotes or valid prior state returns WAIT, enabling same-day retry without poisoning the journal. Prior holdings disappearing from new targets still require quotes. Initial flat state and genuine overlay warmup remain valid; broken/gapped chains are not reset. Snapshot and source provenance remain in journal rows with compact settlement-day details. Paired admission requires serialized invocation and is not a two-file transaction. Funding remains the existing daily approximation; the separate event-accounting API is not integrated by this change.

Verification passed **584 integrated Python tests plus four preservation self-tests**. Independent reviews found no unresolved material issue within the reviewed scope. All 3,805 prior local artifacts, 222 external original inputs, six September 9 result files and 61 gate entries pass preservation checks. The 730-row / 428,150-byte financial ledger retains SHA-256 `710a4f087325bfb408ed8d051963a473e8d8a47b445ffae0238c565aff6dc791` with no new rows. The 288-file source archive was checked against its frozen commit, including identity outside Git. It has not been uploaded or deployed. No scheduler, account request, paper start, financial replay, model fit or holdout run occurred. The report and manual collection procedure are under `docs/funding-capture/`. **Two conditional measured failures and 22 unavailable accounting cases remain; zero validated strategies.**

## Section 95: Fixed factor-floor accounting correction (2026-09-10)

The separately registered eighteen-configuration BTC/ETH factor correction completed on source `27640882822d812c6d0478340495e033a11d3915`, following charter/archive commitacb8c8b and gate/input commit9cedcc4. All18primary cases,90fixed two-sleeve book/index evaluations and18invalid-log shadows are preserved. The37original factor ledger rows represent two runs of the same18configurations plus a halt note. The preserved old engine reproduces all18archived scalar summaries and the sole saved BEST daily stream within1e-10. This establishes numerical parity on current saved caches, while unavailable July input hashes and exact pre-result code/grid provenance remain qualifications.

On the common365-day reporting basis,16/18configurations improve and7/18have positive primary Sharpe. The largest is30-day long/short momentum: old0.2797→corrected0.9851, return65.33%, maximum drawdown12.13%, doubled-execution-cost Sharpe0.9069 and zero-assumed-funding Sharpe1.0100. The historical10/50MA benchmark improves0.7609→0.8585. The20-day Donchian primary0.2884 is fragile: double-cost−0.0101 and zero-funding0.0044. The20/100MA primary remains negative−0.0206 despite its invalid-log shadow+0.0695; no invalid convention is selected.

Every primary sleeve eventually reaches the preserved permanent15%drawdown halt:36/36BTC/ETHsleeves, with238–1157post-halt cash days per benchmark. The1240-day return clock, initial NAV and cash tails remain in all primary metrics. Prices retain the original unverified spot/mixed-provider proxy; funding remains an assumed signed3bp/day rather than realized perpetual events. Stops retain daily opening-exposure funding and threshold-fill assumptions. The50/50mean of separately simulated sleeve returns is a benchmark index, not a pooled executable account. No new standalone adoption gate, champion selection or holdout is introduced; the original BEST remains unchanged.

Verification passed107synthetic/causal/sizing/accounting/registry tests plus6preservation selftests before the run, with independent source and result reviews retained. A first source-fence rejection occurred before input loading or output creation and is preserved; the actual empirical run used clean committed source. The result has290immutable artifacts, and18new ledger records retain all90nested comparisons and18shadows while preserving the original730-row/428150-byte prefix. See `docs/factor-correction/FACTOR_CORRECTION_2026-09-10.md`, the immutable result and final verification records. The earlier22settlement-blocked accounting cases remain unavailable. ENet/overlays, manuscript and VPS are unchanged. **Zero validated strategies.**


## Section 96: Saved forecast inference and factor risk diagnostics (2026-09-10)

Two bounded diagnostic charters were committed at `38d9a67e6ff042cb1fd870877b3e0222f40fdc08` before implementation/results. Both single executions used reviewed source `cd8d9e3f064bb40273ba493e597e371e6f199b66`. All 16 saved ENet comparisons and all 36 primary factor sleeves are complete. There was no model fit, strategy replay, network request, new data purchase or holdout read. The original 748-row financial ledger remains byte-identical; 16 and 36 records are retained in separate forensic ledgers. Full report: `docs/diagnostics-2026-09-10/RESULTS.md`.

The fixed 21-day stationary bootstrap retains physical-clock masks and baseline fallbacks; all 32,000 draws have valid denominators. Volume-loss improvements versus seasonal naive are BTC hourly 38.82% (95% interval 36.99–40.48%), ETH hourly 37.15% (35.22–38.74%), BTC daily 17.16% (13.00–21.40%) and ETH daily 22.37% (18.64–25.81%). Each has Holm-adjusted p=0.007996 across all 16 slots, exceeds the 5% historical effect descriptor, and improves in all three fixed periods. This is conditional inference on saved forecasts, not a fresh model-class or economic validation. Earlier LightGBM scalar comparisons are contextual only; no competitor is selected or retested.

Every mean-return point comparison is worse than zero prediction. Hourly direction edges of 1.7768/1.9119 percentage points miss the original 2-point floor and their Brier intervals cross zero improvement; the required AUC interval is unavailable. BTC variance is worse; ETH hourly variance retains a favorable conditional interval, while daily ETH variance is uncertain. Return, direction and variance formal primary tests remain unavailable because penalized/selected nested-model inference is unresolved; their Holm input is one. No original gate is reversed.

The factor diagnostic reconciles 44,640 sleeve-date rows, including 8,873 nonzero applied exposures. Risk is sized at entries/flips and can subsequently drift: the applied position-weight-times-volatility proxy exceeds its entry reference on 4,458 active sleeve-dates (50.24%). The 10/50 BTC sleeve reaches a maximum nominal proxy of 80.02% versus its 15% entry reference, without breaching the 3× leverage cap. This is not realized account volatility. Of 852 price stops, 726 are followed by same-direction next-day re-entry using the retained sizing reference, 93 by opposite-direction entry, 25 by permanent halt and 8 by a flat date. Overlapping configuration-date counts are descriptive, not independent trials.

All 36 sleeves reach the preserved permanent halt. Thirty-five cross the drawdown threshold before exit charges; ETH 30-day momentum alone first crosses afterward, from 14.999356% to 15.032114%. The four previously discussed momentum/MA examples have peak-to-halt losses dominated by gross market losses, so cost removal alone does not explain their failures. Signed target-change and maintenance turnover reconcile with explicit netting; stop-successor charges are a subset of opening charges and must not be counted twice.

The previous Section 95/report cash-tail range of 238–1,157 days contains a prose off-by-one. The unchanged traces establish 238–1,158 already-halted cash dates; ETH 180-day momentum halts January 28, 2022 and retains January 29, 2022–March 31, 2025 as 1,158 cash dates. No old metric, halt date or artifact is changed by this addendum.

Independent source reviews passed and 137 relevant tests passed before execution (57 forecast, 80 factor). Result and preservation evidence is retained under the report's verification directory. A future risk-policy comparison must explicitly define sizing/stop interaction, comparison scope and fresh validation before calculation. Forecast-to-execution economics likewise needs its own registered experiment. The spent holdouts remain spent, the 22 settlement-blocked cases remain deferred, and zero strategies are validated.


## Section 97: Fixed factor sizing and stop re-entry comparison (2026-09-10)

Registration `risk_policy_2026_09_10` was committed at `67eb720305ccddff266df396e61d03dc2fa1985e` before implementation. Reviewed source `2dfb047d5c2f2ff821706736eb9f5508a14e7304` was committed and pushed before one execution. All eighteen original factor configurations were crossed with saved versus daily volatility sizing and immediate versus new-target-episode price-stop re-entry. The original signals and saved target direction decisions,20-return/sqrt 252 volatility estimator,0.15 sizing numerator,3× cap,3% threshold price stop,15% permanent drawdown halt and four cost assumptions were fixed. Raw zero/opposite targets release a waiting block; same-sign resizing or underlying signal changes do not. No historical or prospective validation gate was introduced.

All 72 primary identities,288 cost-specific indices,576 independently simulated sleeve books and 72 frozen invalid-log shadows completed. Before alternatives, all 144 original control traces and 72 return frames reproduced the pinned references. Every return series retains all 1,240 dates and initial NAV; the BTC/ETH mean-return index remains distinct from a pooled executable account. Fifty-four direct contrasts and 54 descriptive factorial effects are retained. The financial ledger appends exactly 72 identities to the unchanged 748-row/569,325-byte prefix, giving 820 rows. Eighteen new rows are repeated controls and 54 are changed policies; the counts are an audit inventory, not independent hypotheses.

For original control/daily sizing/waiting/both, positive primary return and Sharpe counts are 7/6/10/10 out of 18. Daily sizing improves average and compounded returns in 7/18; either waiting arm improves 14/18. Daily resizing removes all observed applied nominal-risk-budget excesses (4,458 original active dates versus 0 in either daily arm), but alone leaves all 36 sleeves halted. Active sleeve-date counts are 8,873/7,750/7,739/8,618, price stops 852/789/433/474, and permanent halts 36/36/18/18. These are descriptive counts on correlated books; the nominal risk proxy is not realized account volatility.

Waiting's apparent halt improvement is heavily affected by flatness. All 14 long-only sleeves avoid halts under both waiting arms but retain only 118 active dates out of 17,360;16,671 dates are waiting and 571 otherwise flat. Every long-only sleeve remains gross and net negative. All seven long-only index losses become smaller, accounting for half of the fourteen improved indices. Only four of the 22 remaining sleeves avoid the halt. Four previously negative long/short configurations—90-day and 180-day momentum,20/100 and 50/200 moving-average crossovers—become positive under either waiting policy across all four cost assumptions; cross-asset momentum becomes negative. No champion is selected from these exposed comparisons.

The earlier favorable examples worsen under each changed arm. Thirty-day long/short momentum's primary Sharpe is 0.985/0.856/0.769/0.644 across the four arms;10/50 moving-average crossover is 0.859/0.203/0.712/0.540. Doubled-cost positive Sharpe counts are 6/5/9/9; zero-funding counts 7/6/10/10. Invalid log accounting changes one primary sign classification (20/100 MA control−0.020635→+0.069505) and two direct Sharpe-difference orderings; no invalid convention is selected.

Verification passed 135 new synthetic tests plus the retained related regressions, and 13,780,216 independent saved-evidence assertions across all 576 books. Prior gate objects, immutable tracked artifacts and 270 external provenance inputs remain preserved. The result SHA-256 is `245ad6a30022a4ab7f115b1acb52d33dc5537ce43d8c0b6850e5ec25391d1006`; all 2,093 output files are retained. Interpretation and full tables are in `docs/risk-policy-2026-09-10/INTERPRETATION.md` and `RESULTS.md`. The separately committed fresh-validation plan requires a later exact registration, venue economics, power/gates and prospective dates after operational admission; no future validation has started. Proxy prices, assumed signed daily funding, threshold fills, spent history and separate-sleeve capital remain qualifications. No network data request, paid purchase, provider contact, model fit, holdout read, paper restart or VPS change occurred. Settlement recovery remains deferred. **Zero validated strategies.**

## Section 98: Current dated BTC/ETH carry feasibility — negative conditional economics (2026-09-10)

The user's specified objective is positive net returns with little crypto-price exposure, initially at approximately 1,000 and potentially 10,000 capital, using Binance and possibly Bitrue. A fixed public-book measurement and synthetic full-hedge accounting example were registered at `57149c6bfeefe3a1a76e5103d21dbd598d10080c`. Reviewed execution source `62bcf8bf4bdb2c57e8b78d4443427863e287d692` was committed and pushed before the single capture. The separate `dated_carry_feasibility_2026_09_10` gate preserves all 40 earlier gate objects and does not append financial trials. The original 820-row ledger remains byte-identical.

Sixteen public Binance requests retained metadata/clocks and three scheduled depth batches from September 10, 2026 16:11:14 through 16:12:16 UTC. The earliest eligible conventional linear USDT expiry rule selected BTCUSDT_260925 and ETHUSDT_260925, expiring September 25 at 08:00 UTC. All 48 entry cases and 432 terminal scenarios are calculable and retained. Each asset independently uses the full 1,000/10,000 USDT budget; the funds are assumed already on venue. Current depth does not establish fills, and absent spot event timestamps leave spot freshness unverified. Bitrue remains documentation-only, with eligible dated-product availability unverified.

All 432 modeled net cash outcomes are negative under the registered fee assumptions. For full futures-notional reserve, base fees, unchanged terminal index and no spot/index exit mismatch, cash profits across three observations are BTC −0.5780 to −0.5468 and ETH −0.9028 to −0.8009 USDT at 1,000 capital; BTC −6.4936 to −6.0496 and ETH −9.8956 to −8.2563 at 10,000. The observed matched entry premium is approximately 10–18 basis points, below approximately 30 basis points of modeled four-fee drag in these reference cases. All 48 entries are budget-limited. Increasing capital does not repair the negative unit economics.

All four asset/capital necessary screens fail, with zero positive outcomes in each nine-cell conjunction: three snapshots × half/unchanged/doubled terminal index, full reserve, doubled fees and 10bp adverse spot exit. These are conditional arithmetic scenarios, not independent trials or evidence of historical robustness. Illustrative 0%, 3% and 5% annual cash benchmarks remain separate and are not the user's chosen hurdle. Negative base cash profits do not depend on subtracting opportunity cost.

Spot principal, base-denominated purchase commission, quantity rounding/residual exposure, futures entry fees, collateral and terminal fees are accounted in explicit cash/base units. Futures entry notional supplies no cash proceeds. The synthetic invented-price hedge example reconciles down/flat/up terminal states independently and exposes the invalid log convention; its positive example profits are not market results. All 72 one-third-reserve scenarios with doubled terminal index show insufficient terminal futures reserve. All 216 full-reserve scenarios have positive terminal reserve within this grid, which does not establish maintenance-margin compliance or pathwise survival.
