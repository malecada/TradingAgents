...
def runtime_hashes() -> dict[str, str]:
    ...
    extended = Path(__file__).parent.parent / "research_extended"
    own.update({"extended/" + p.name: digest(p.read_bytes()) for p in sorted(extended.glob("*.py"))})
    return own
...
def admit(*, root, registration, experiment, source, design_source=None, bindings=None,
    ...
    extension = None
    if "budget_book_grant" in exp:
        from .grant import validate
