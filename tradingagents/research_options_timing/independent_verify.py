...
PACKAGES = ('research', 'research_amended', 'research_extended', 'research_spread', 'research_options_capture', 'research_options_timing')
...
def reconstruct(root, claim, now):
    ...
    require(set(protocol) == {'schema_version', 'observation_window', 'worker_lease', 'analysis_inputs', 'resources', 'hourly_acquisition_delay_ms'} and type(protocol['schema_version']) is int and protocol['schema_version'] == 3 and type(protocol['hourly_acquisition_delay_ms']) is int and protocol['hourly_acquisition_delay_ms'] == 2000, 'prospective v3 timing protocol')
    ...
    keys = {'schema_version','target_experiment','program_id','family_id','mechanism_id','original_budget','prior_attempts','increment','effective_budget','target_contract_sha256','history_source','prior_effective_budget','parent_experiment','consumed_episode_grant','closed_parent','prior_claims','options_prior_ids','review','preflight'}
    require(set(grant) == keys, 'grant denominator')
    for key, value in {'schema_version':2, 'original_budget':4, 'prior_attempts':1, 'increment':1, 'effective_budget':6, 'prior_effective_budget':5}.items():
    ...
    require(isinstance(prior, dict) and len(prior) == 20, 'all twenty predecessors')
    parent = 'options-episode-20260911'
    require(exp.get('parent') == parent and grant['parent_experiment'] == parent, 'fixed failed predecessor')
    cert = reference(root, grant['closed_parent'], source, parse=True)
    reference(root, grant['closed_parent'], design)
    require(cert.get('original_grant') == grant['consumed_episode_grant'], 'consumed original grant binding')
    ...
    require(len(options) == 4 and options == grant['options_prior_ids'] and all(old_claims[k]['family'] == family for k in options), 'four prior options claims and unchanged family')
    ...
    require(set(review) == {*common, 'decision', 'independent_review'} and review['decision'] == 'approve-single-timing-corrected-options-episode' and all(review[k] == v for k,v in common.items()), 'independent grant approval')
...
def verify(*, root, now_utc, quiescence_evidence=None):
    ...
    if not terminals:
        return {'status':'active','claim_sha256':claimed,'quiescence':'externally-reviewed' if quiet_at else 'unavailable','history_count':20,'scientific_validation':False}
    ...
    return {'status':status,'claim_sha256':claimed,'history_count':20,'quiescence':'externally-reviewed','scientific_validation':False}
