...
from .schedule import ASSETS, DAY, HOUR, WINDOW, calendar, nominal_ms
...
def _read(receipts, group, spec, entry_ms):
    return decode(receipts.get(group, {})[spec['id']], spec['request'],
                  scheduled_ms=nominal_ms(entry_ms,spec), cap=spec['body_cap'], hourly=group=='known')
...
def normalize(*, asset, entry_ms, exit_ms, receipts, bootstrap_receipt=None):
    ...
    for h in range(hours+1):
        ...
        try:
            value, _, meta = _read(receipts,'known',spec,entry_ms)
    ...
    for h in range(hours):
        ...
        try:
            ...
            if bounds[0] < nominal_ms(entry_ms,spec)-DRIFT_MS or bounds[1] > meta['controller_retrieval_ms']+DRIFT_MS:
    ...
    for group in ('daily','final'):
        for spec in slots[group]:
            ...
            try:
                rows, _, meta = _read(receipts,group,spec,entry_ms)
