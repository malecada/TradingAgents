...
def package_assignment(package,expected):
    ...
    if set(spec)!=keys or type(spec['schema_version']) is not int or spec['schema_version']!=1 or spec['authority']!='raw-worker-only' or spec['target']!='options-timing-20260915':raise ValueError('raw assignment schema/authority')
    ...
    if type(protocol.get('schema_version')) is not int or protocol['schema_version']!=3 or type(protocol.get('hourly_acquisition_delay_ms')) is not int or protocol['hourly_acquisition_delay_ms']!=2000:raise ValueError('fixed timing successor protocol required')
    ...
    for name in ('journal','schedule','adapter'):
        relative=f'tradingagents/research_options_timing/{name}.py'
...
def prepare(*,package,data,expected_assignment_sha256,require_sealed=True):
    ...
    for hour in range(max_hour+1):
        ...
        for asset in ASSETS:
            ...
            try:
                clock,_,clockmeta=adapter.decode(batch['options-time'],adapter._recipe(known_specs['options-time']),scheduled_ms=at,hourly=True)
                bounds=adapter._clock(clock,clockmeta);key=asset.lower()+'-index';source=known_specs[key]
                value,raw,meta=adapter.decode(batch[key],adapter._recipe(source),scheduled_ms=at,hourly=True)
