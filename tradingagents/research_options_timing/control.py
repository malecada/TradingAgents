...
Grant schema is checked by _admit; episode_protocol contains version3 with fixed 2000ms hourly acquisition delay,
...
def runtime_hashes():
    ...
    return {package+'/'+p.name:file_sha(p) for package in ('research','research_amended','research_extended','research_spread','research_options_capture','research_options_timing') for p in sorted((base/package).glob('*.py'))}
...
def inventory(root,own=None,allow_active=False):
    ...
    for directory in sorted((root/'research_runs').iterdir()):
        ...
        if directory.is_symlink() or not directory.is_dir():raise ControlError('unsafe historical directory')
        claim_path=directory/'claim.json'
        if not stat.S_ISREG(claim_path.lstat().st_mode) or claim_path.stat().st_size>4*1024**2:raise ControlError('unsafe historical claim')
        ...
        if directory.name == 'options-episode-20260911':
            limits=claim['episode_protocol']['resources']
            member_sizes(directory/'control',{'source-binding.json','stop-ack.json','analysis-intent.json','stop-pending.json'},limits['control_total_bytes'],limits['control_total_bytes'])
            controls={p.name:file_sha(p) for p in (directory/'control').iterdir()}
            if controls!=terminal.get('control_sha256'):raise ControlError('historical options control inventory changed')
            items[directory.name]['control_sha256']=controls
    return items,claims
...
def _admit(root,registration,source,design_source,now,own=None):
    ...
    if set(protocol)!={'schema_version','observation_window','worker_lease','analysis_inputs','resources','hourly_acquisition_delay_ms'} or type(protocol['schema_version']) is not int or protocol['schema_version']!=3 or type(protocol['hourly_acquisition_delay_ms']) is not int or protocol['hourly_acquisition_delay_ms']!=2000:raise ControlError('episode protocol schema')
    ...
    fields={'schema_version','target_experiment','program_id','family_id','mechanism_id','original_budget','prior_attempts','increment','effective_budget','target_contract_sha256','history_source','prior_effective_budget','parent_experiment','consumed_episode_grant','closed_parent','prior_claims','options_prior_ids','review','preflight'}
    if set(grant)!=fields:raise ControlError('target-only grant schema')
    for key,value in [('schema_version',2),('original_budget',4),('prior_attempts',1),('increment',1),('effective_budget',6),('prior_effective_budget',5)]:
    ...
    if exp.get('parent')!=parent or grant['parent_experiment']!=parent:raise ControlError('fixed failed parent required')
    cert=json.loads(reference(root,grant['closed_parent'],source));reference(root,grant['closed_parent'],design_source)
    ...
    if len(options)!=4 or grant['options_prior_ids']!=options:raise ControlError('four spent options claims required')
    ...
    if set(approval)!={*common,'decision','independent_review'} or approval['decision']!='approve-single-timing-corrected-options-episode' or any(approval[k]!=v for k,v in common.items()):raise ControlError('explicit book approval missing')
