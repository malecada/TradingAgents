"""Independent frozen twentieth-protocol reconstruction, without controller admission.
...
def reconstruct(root, claim, now, successor=None):
    ...
    for directory in sorted((root / 'research_runs').iterdir()):
        ...
        if directory.name not in prior:
            if old['family']['mechanism_id'] == MECHANISM:
                require(successor is not None and directory.name == 'options-timing-20260915' and old['experiment_id'] == successor['experiment_id'], 'unbound same-family descendant')
                require(old['source'] == successor['source'] and old['design_source'] == successor['design_source'] and old['experiment'].get('episode_book_grant') == successor['grant'], 'successor grant/source identity')
...
def _verify_original(*, root, now_utc, quiescence_evidence=None, successor=None):
...
def verify_parent(*, root, certificate, source, design_source, now_utc, successor=None):
    """Reconstruct the frozen twentieth protocol at its explicit closed boundary.

    The full live directory is examined. A successor is recognized only by its
    caller-supplied, committed exact grant/source identities; no filesystem view
    is filtered, copied, or substituted. This checks evidence bindings, not
    actual remote process death from declarations.
    """
    root = Path(root).resolve()
    cert = reference(root, certificate, source, parse=True)
    reference(root, certificate, design_source)
    keys = {'schema_version','experiment_id','claim_sha256','terminal_sha256','control_sha256','output_sha256','closed_at','quiescence_evidence','closure_review','prior_claims_sha256','original_grant'}
    require(isinstance(cert, dict) and set(cert) == keys and type(cert['schema_version']) is int and cert['schema_version'] == 1 and cert['experiment_id'] == TARGET, 'closed parent certificate grammar')
    ancestor(root, cert['quiescence_evidence']['commit'], design_source)
    if successor is not None:
        require(set(successor) == {'experiment_id','source','design_source','grant'} and successor['experiment_id'] == 'options-timing-20260915', 'successor boundary grammar')
        require(successor['source'] == source and successor['design_source'] == design_source, 'successor boundary source')
        extension = reference(root, successor['grant'], source, parse=True)
        reference(root, successor['grant'], design_source)
        require(extension.get('closed_parent') == certificate and extension.get('parent_experiment') == TARGET and extension.get('target_experiment') == successor['experiment_id'], 'explicit successor certificate binding')
        expected_keys={'schema_version','target_experiment','program_id','family_id','mechanism_id','original_budget','prior_attempts','increment','effective_budget','target_contract_sha256','history_source','prior_effective_budget','parent_experiment','consumed_episode_grant','closed_parent','prior_claims','options_prior_ids','review','preflight'}
        require(set(extension)==expected_keys and all(type(extension[k]) is int and extension[k]==v for k,v in {'schema_version':2,'original_budget':4,'prior_attempts':1,'prior_effective_budget':5,'increment':1,'effective_budget':6}.items()), 'authorized successor fixed sixth accounting')
        require(extension['family_id']==FAMILY and extension['mechanism_id']==MECHANISM and isinstance(extension['prior_claims'],dict) and len(extension['prior_claims'])==20 and extension['consumed_episode_grant']==cert['original_grant'], 'authorized successor family/history')
        approval=reference(root,extension['review'],source,parse=True);reference(root,extension['review'],design_source)
        common={'target_experiment':successor['experiment_id'],'target_contract_sha256':extension['target_contract_sha256'],'prior_claims_sha256':digest(canonical(extension['prior_claims']))}
        require(set(approval)=={*common,'decision','independent_review'} and approval['decision']=='approve-single-timing-corrected-options-episode' and all(approval[k]==v for k,v in common.items()), 'authorized successor review')
        reference(root,approval['independent_review'],source);reference(root,approval['independent_review'],design_source)

    result = _verify_original(root=root, now_utc=now_utc, quiescence_evidence=cert['quiescence_evidence'], successor=successor)
    directory = root/'research_runs'/TARGET
    claim = json_file(directory/'claim.json')
    terminal = json_file(directory/'failed.json')
    grant = reference(root, cert['original_grant'], claim['source'], parse=True)
    require(cert['original_grant'] == claim['episode_book_grant'], 'original fifth grant binding')
    reference(root, cert['original_grant'], source)
    require(cert['claim_sha256'] == file_hash(directory/'claim.json') and cert['terminal_sha256'] == file_hash(directory/'failed.json'), 'closed parent claim/terminal hash')
    require(cert['control_sha256'] == terminal['control_sha256'] and cert['output_sha256'] == terminal['output_sha256'] and cert['closed_at'] == terminal['ended_at'], 'closed parent exact inventory/clock')
    require(cert['prior_claims_sha256'] == digest(canonical(grant['prior_claims'])), 'original nineteen boundary binding')
    require(result['status'] == 'failed' and terminal['cell_count'] == terminal['unavailable_count'] == 8 and {c['id'] for c in terminal['cells']} == set(claim['experiment']['cells']) and all(c['status'] == 'unavailable' for c in terminal['cells']) and not terminal['output_sha256'] and 'analysis-intent.json' not in terminal['control_sha256'], 'fixed failed eight unavailable parent')
