...
def nominal_ms(entry_ms, item):
    """Recover nominal time without moving action/freshness with acquisition."""
    validate_entry(entry_ms)
    name = item['id']
    if name.startswith('h'):
        match = re.fullmatch(r'h(\d{4})-(?:options-time|futures-time|(?:btc|eth)-(?:index|perp-depth|perp-mark|(?:call|put)-(?:depth|mark)))', name)
        if match is None or int(match[1]) > 1056:
            raise ValueError('fixed hourly role identity required')
        nominal = entry_ms + int(match[1])*HOUR
        if item['scheduled_ms'] != nominal+ACQUISITION_DELAY_MS or item['deadline_ms'] != nominal+WINDOW:
            raise ValueError('fixed hourly acquisition/action clocks required')
        return nominal
    if item['deadline_ms'] != item['scheduled_ms']+WINDOW:
        raise ValueError('fixed non-hourly source window required')
    return item['scheduled_ms']
...
def calendar(entry_ms, selection=None):
    ...
    for group in ('known', 'selected'):
        for item in out[group]:
            item['scheduled_ms'] += ACQUISITION_DELAY_MS
    for d in range(45):
