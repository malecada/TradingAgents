...
def evaluate_cell(run,cell,examples,scaler,features,model_config,training_config,provenance,*,expected_test_mask,feature_binding,output_directory=None,feature_binding_input=None,feature_binding_output=None,continuation=None,completed_fit=None):
    ...
    config_identity=digest(canonical_bytes({'model':model_config,'training':training_config,'cell':{k:cell[k] for k in ('id','arm','task','seed','fold','variant')},'feature_binding':feature_binding,'scaler':asdict(scaler)}))
