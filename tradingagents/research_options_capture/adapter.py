...
from .schedule import ASSETS, DAY, HOUR, WINDOW, request
...
def decode(receipt,recipe,*,scheduled_ms,cap=8192):
    """Reconstruct complete HTTP bytes and controller availability, not a fill."""
    ...
    if type(start) is not int or type(end) is not int or not scheduled_ms<=start<=end<=scheduled_ms+WINDOW:raise ValueError('fixed observation window')
    ...
    if not scheduled_ms<=anchor<=start or meta['controller_deadline_ms']!=scheduled_ms+WINDOW or deadline!=am+(scheduled_ms+WINDOW-anchor)*1000000:raise ValueError('raw controller deadline anchor')
    if not am<=sm<=wm<=em<=deadline or not start<=we<=end+1 or em-am>WINDOW*1000000:raise ValueError('raw clock chronology/deadline')
...
def select_initial(metadata_receipt,known_receipts,*,entry_ms):
    ...
    try:
        ...
        for key,spec in specs.items():
            try:
                value,raw,meta=decode(known_receipts[key],_recipe(spec),scheduled_ms=entry_ms)
...
def assemble_hour(*,entry_ms,hour,selection,receipts):
    ...
    for spec in BATCH.requests(symbols):
        ...
        try:
            ...
            _,_,meta=decode(receipt,_recipe(spec),scheduled_ms=at)
            item.update(meta)
