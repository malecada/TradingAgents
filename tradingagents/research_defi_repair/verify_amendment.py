"""Reconstruct amendment evidence without importing admission or lifecycle."""
import hashlib
...
def earlier_claim(previous, target):
    """As-of reconstruction only; admission still counts every current claim."""
    def clock(value):
        stamp = datetime.fromisoformat(value)
        if stamp.tzinfo is None or stamp.utcoffset().total_seconds() != 0:
            raise ValueError('amendment claim clock must be explicit UTC')
        return stamp
    a, b = clock(previous['started_at']), clock(target['started_at'])
    if a == b:
        raise ValueError('ambiguous equal-time same-family claim')
    return a < b
...
def check(directory, claim, registered, blob):
    ...
    for child in sorted(directory.parent.iterdir()):
        ...
        if previous_claim['family']['mechanism_id'] == family['mechanism_id']:
            if not earlier_claim(previous_claim, claim):
                continue
            if 'budget_amendment' in previous_claim['experiment'] or 'budget_amendment' in previous_claim:
                raise ValueError('chained or duplicate amendment')
