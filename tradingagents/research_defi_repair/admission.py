...
def admit(*, root, registration, experiment, source, design_source=None, bindings=None,
    ...
    """
    if experiment != 'defi-depth-r1-20260915' or registration != 'research/defi-depth-2026-09-15/gates-r1-v2.json':
        raise ValueError('this additive runtime admits only the exact DeFi R1 route')
    ...
    amendment = None
    if "budget_amendment" in exp:
        from .amendment import validate
        amendment = validate(root=root, source=source, design_source=design_source, spec=spec,
                             experiment=experiment, relevant=relevant, own_claim=_own_claim, committed=_committed)
    effective_budget = family["attempt_budget"] if amendment is None else amendment["effective_budget"]
    ...
    return Admission(root, registration, experiment, source, digest(raw), spec, exp, family, windows, ready,
                     design_source, bindings, bindings_sha256, resolved_inputs, amendment)
