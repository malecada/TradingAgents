...
def build(*,root,destination,data_root,host_identity,now_utc):
    ...
    if data_root!='/opt/thesis-research/options-timing-20260915/data':raise ValueError('sole frozen target data root required')
    ...
    for member in sorted(worker.PACKAGE_FILES):
        ...
        if tracked.startswith('tradingagents/research_options_timing/'):
            pin=claim['experiment']['runtime_hashes'].get('research_options_timing/'+Path(tracked).name)
