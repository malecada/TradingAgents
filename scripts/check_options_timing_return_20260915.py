...
from tradingagents.research_options_timing import analysis,returned
from tradingagents.research_options_timing.schedule import ASSETS,DAY,HOUR
from tests.research.test_options_timing_adapter import metadata,T
...
def main():
    ...
    report['source_sha256']={str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in sorted((ROOT/'tradingagents/research_options_timing').glob('*.py'))}
